...
local function SelfDefensives()
    ...
    if (HPLoosePerSecond >= ShieldoftheRighteousHPLost or Player:ActiveMitigationNeeded()) and A.ShieldoftheRighteous:IsReady(player) and Unit(player):HasBuffs(A.ShieldoftheRighteous.ID, true) == 0 and Unit(player):HasBuffs(A.ArdentDefender.ID, true) == 0 and Unit(player):GetRealTimeDMG(3) > 0 then 
        return A.ShieldoftheRighteous
    end 
	
	-- LightoftheProtector 
	local LightoftheProtectorHPLost = GetToggle(2, "LightoftheProtectorHPLost")
	local LightoftheProtectorHPLost = GetToggle(2, "LightoftheProtectorHP")
    if not A.HandoftheProtector:IsSpellLearned() and (HPLoosePerSecond >= LightoftheProtectorHPLost or A.LightOfProtector:PredictHeal("LightOfProtector", "player")) and A.LightoftheProtector:IsReady(player) and Unit(player):HealthPercent() <= LightoftheProtectorHP and Unit(player):GetRealTimeDMG(3) > 0 then 
	...
	then 
        ...
        then
            ...
            if A.ShieldoftheRighteous:IsReadyByPassCastGCD(player) and Unit(player):HasBuffs(A.ShieldoftheRighteous.ID, true) == 0 and Unit(player):HasBuffs(A.ArdentDefender.ID, true) == 0 then  
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        if combatTime == 0 and unit ~= "mouseover" then
            ...
            -- consecration
            if A.Consecration:IsReady("player") and Consecration() <= 3 then
		...
		if inCombat and unit ~= "mouseover" then
            ...
            if A.ShieldoftheRighteous:IsReady(unit) and ((Unit("player"):HasBuffs(A.AvengersValorBuff.ID, true) > 0 and A.ShieldoftheRighteous:GetSpellChargesFrac() >= 2.5) and (A.Seraphim:GetCooldown() > A.GetGCD() or not A.Seraphim:IsSpellLearned())) then
            ...
            if A.ShieldoftheRighteous:IsReady(unit) and A.ShieldoftheRighteous:GetSpellChargesFrac() >= 1.5 and ((Unit("player"):HasBuffs(A.AvengingWrathBuff.ID, true) > 0 and not A.Seraphim:IsSpellLearned()) or Unit("player"):HasBuffs(A.SeraphimBuff.ID, true) > 0 and Unit("player"):HasBuffs(A.AvengersValorBuff.ID, true) > 0) then
                return A.ShieldoftheRighteous:Show(icon)
            end
