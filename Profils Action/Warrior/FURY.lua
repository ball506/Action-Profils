	...
	local function EnemyRotation(unitID)
        ...
        if unitID ~= "mouseover" and A.BurstIsON(unitID) then             
			...
			if inMelee then 
			    ...
			    if A.AshvanesRazorCoral:IsExists() and A.AshvanesRazorCoral:IsReady(unitID) and TrinketON() and (not Unit(unitID):HasDeBuffs(A.RazorCoralDebuff.ID) <= 0 or (Unit(unitID):HasDeBuffs(A.RazorCoralDebuff.ID) > 0 and Unit(unitID):HealthPercent() <= 30 and Unit(unitID):TimeToDie() >= 10)) then
