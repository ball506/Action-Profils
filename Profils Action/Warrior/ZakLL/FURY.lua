...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unitID)
        ...
        if A.Rampage:IsReadyByPassCastGCD(unitID) and A.Rampage:AbsentImun(unitID, Temp.TotalAndPhys) and A.Recklessness:GetCooldown() < 3 then
        ...
        if A.FuriousSlash:IsReady(unitID) and A.FuriousSlash:AbsentImun(unitID, Temp.TotalAndPhys) and not Unit("player"):HasHeroism() and Unit("player"):HasBuffs(A.FuriousSlashBuff.ID, true) < 4 then
            return A.FuriousSlash:Show(icon)
        end  
		
		-- out of range alert in combat
		if (Unit(unitID):IsBoss() or Unit(unitID):IsDummy()) and Unit(unitID):GetRange() > 5 and Unit("player"):CombatTime() > 0 then
		    -- Notification					
       	    Action.SendNotification("ALERT : Out of range from target", A.Rampage.ID)		
            PlaySound(8959) -- by SoundKitID		
            return			
		end
        ...
        if A.GetToggle(2, "RampageLogic") == "Simcraft Logic" then
            ...
            if A.Rampage:IsReadyByPassCastGCD(unitID) and A.Rampage:AbsentImun(unitID, Temp.TotalAndPhys) and ((Unit("player"):HasBuffs(A.RecklessnessBuff.ID, true) > 0 or  Unit("player"):HasBuffs(A.MemoryofLucidDreams.ID, true) > 0) or 
        ...
        elseif A.GetToggle(2, "RampageLogic") == "High Priority Logic" then
            if A.Rampage:IsReadyByPassCastGCD(unitID) and A.Rampage:AbsentImun(unitID, Temp.TotalAndPhys) then
                return A.Rampage:Show(icon)
            end
        end    
        ...
        if A.Bloodthirst:IsReadyByPassCastGCD(unitID) and A.Bloodthirst:AbsentImun(unitID, Temp.TotalAndPhys) and (Unit("player"):HasBuffs(A.EnrageBuff.ID, true) == 0 or A.ColdSteelHotBlood:GetAzeriteRank() > 1) then
        ...
        --bloodthirst
        if A.Bloodthirst:IsReadyByPassCastGCD(unitID) and A.Bloodthirst:AbsentImun(unitID, Temp.TotalAndPhys) then
            return A.Bloodthirst:Show(icon)
        end
        
        --raging_blow,if=talent.carnage.enabled|(talent.massacre.enabled&rage<80)|(talent.frothing_berserker.enabled&rage<90)
        if A.RagingBlow:IsReady(unitID) and A.RagingBlow:AbsentImun(unitID, Temp.TotalAndPhys) and 
		(
       ...
       -- if A.FuriousSlash:IsReady(unitID) and A.FuriousSlash:IsSpellLearned() and A.FuriousSlash:AbsentImun(unitID, Temp.TotalAndPhys) then
       --     return A.FuriousSlash:Show(icon)
       -- end
