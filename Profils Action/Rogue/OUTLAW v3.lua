...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        if not inCombat and Unit(unit):IsExists() and unit ~= "mouseover" then 
			...
			if OpenerMode == "AUTO" and (Pull > 0 and Pull < 1 or not A.GetToggle(1, "DBM")) and
			(
			...
			elseif OpenerMode == "CHEAPSHOT" then
			    return A.CheapShot:Show(icon)
			else
			    return A.Ambush:Show(icon)
			end			    
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if (A.GetToggle(2, "AoE") or isMulti) and CanCast then 
                ...
                if A.BloodoftheEnemy:AutoHeartOfAzerothP(unit) and Action.GetToggle(2, "BotEAoEBurst") and Action.GetToggle(1, "HeartOfAzeroth") and 
                (
                    GetByRange(A.GetToggle(2, "BladeFlurryTargets"), A.GetToggle(2, "BladeFlurryRange")) 
                    and 
            ...
            if A.BurstIsON(unit) and CanCast then
                ...
                if A.BloodoftheEnemy:AutoHeartOfAzerothP(unit) and Action.GetToggle(1, "HeartOfAzeroth") and A.BetweentheEyes:GetCooldown() == 0 and RtB_Buffs > 0 then
                    return A.BloodoftheEnemy:Show(icon)
                end
			...
			then
                ...
                if A.BetweentheEyes:IsReady(unit) and Unit(unit):GetRange() <= 20 and Unit(unit):HasDeBuffs(A.Blind.ID, true) == 0 and
				(
				...
				--print(RtB_BuffRemains)
                -- roll_the_bones,if=buff.roll_the_bones.remains<=3|variable.rtb_reroll
                if A.RolltheBones:IsReady("player") and CanCast and 
				(
                ...
                if A.Dispatch:IsReady(unit) and Unit(unit):GetRange() <= 12 then
            ...
            if CanCast then
                ...
                if A.SinisterStrike:IsReady(unit) and Unit(unit):GetRange() <= 12 then
                    return A.SinisterStrike:Show(icon)
                end
            end
...
local function ArenaRotation(icon, unit)
    if A.IsInPvP and (A.Zone == "pvp" or A.Zone == "arena") and not Player:IsStealthed() and not Player:IsMounted() then
        ...
        if unit == "arena1" and (Unit("player"):GetDMG() == 0 or not Unit("player"):IsFocused("DAMAGER")) then 
            ...
            if A.NetherWard:IsReady(unit) and A.NetherWard:IsSpellLearned() and Action.ShouldReflect(unit) and EnemyTeam():IsCastingBreakAble(0.25) then 
