...
Action[ACTION_CONST_MONK_BREWMASTER] = {
    ...
    Provoke                                      = Create({ Type = "Spell", ID = 115546    }),    
    SummonBlackOxStatue                         = Create({ Type = "Spell", ID = 115315, isTalent = true     }),
    ProvokeSummonBlackOxStatue                = Create({ Type = "Spell", ID = 115546, Color = "PINK", Texture = 115315 }),
    Admonishment                             = Create({ Type = "Spell", ID = 207025, isTalent = true    }),    -- PvP Talent Provoke 
    Detox                                      = Create({ Type = "Spell", ID = 218164    }),
    TigersLust                                     = Create({ Type = "Spell", ID = 116841, isTalent = true    }),
    Vivify                                      = Create({ Type = "Spell", ID = 116670    }),
    -- Defensives    
    IronskinBrew                              = Create({ Type = "Spell", ID = 115308     }), -- on simcraft it is Brews
    PurifyingBrew                            = Create({ Type = "Spell", ID = 119582     }),
    HealingElixir                              = Create({ Type = "Spell", ID = 122281, isTalent = true     }),
    ZenMeditation                              = Create({ Type = "Spell", ID = 115176     }),
    Guard                                      = Create({ Type = "Spell", ID = 115295, isTalent = true     }),    
    DampenHarm                                = Create({ Type = "Spell", ID = 122278, isTalent = true     }),
    FortifyingBrew                            = Create({ Type = "Spell", ID = 115203     }),            
    -- Rotation     
    BlackoutStrike                            = Create({ Type = "Spell", ID = 205523     }),
    BlackOxBrew                               = Create({ Type = "Spell", ID = 115399, isTalent = true     }),    
    BreathofFire                              = Create({ Type = "Spell", ID = 115181     }),
    CracklingJadeLightning                      = Create({ Type = "Spell", ID = 117952     }),
    ChiBurst                                  = Create({ Type = "Spell", ID = 123986, isTalent = true     }),
    ChiWave                                   = Create({ Type = "Spell", ID = 115098, isTalent = true     }),    
    ExpelHarm                                  = Create({ Type = "Spell", ID = 115072     }),
    InvokeNiuzaotheBlackOx                    = Create({ Type = "Spell", ID = 132578, isTalent = true     }),    
    KegSmash                                  = Create({ Type = "Spell", ID = 121253     }),    
    RushingJadeWind                           = Create({ Type = "Spell", ID = 116847, isTalent = true     }),    
    TigerPalm                                 = Create({ Type = "Spell", ID = 100780     }),
    -- Auras 
    BreathofFireDotDebuff                    = Create({ Type = "Spell", ID = 123725, Hidden = true }), 
    BlackoutComboBuff                       = Create({ Type = "Spell", ID = 196736, Hidden = true }), 
    SpecialDelivery                           = Create({ Type = "Spell", ID = 196730, isTalent = true, Hidden = true }),    
    LightBrewing                              = Create({ Type = "Spell", ID = 196721, isTalent = true, Hidden = true }),
    -- Movememnt    
    Roll                                    = Create({ Type = "Spell", ID = 109132    }),
    ChiTorpedo                                = Create({ Type = "Spell", ID = 115008, isTalent = true    }),
    TranscendenceTransfer                    = Create({ Type = "Spell", ID = 119996    }), -- not usable in APL but user can Queue it
    -- PvP
    DoubleBarrel                            = Create({ Type = "Spell", ID = 202335, isTalent = true    }),
    MightyOxKick                            = Create({ Type = "Spell", ID = 202370, isTalent = true    }),
    AvertHarm                                = Create({ Type = "Spell", ID = 202162, isTalent = true    }),    
    CraftNimbleBrew                            = Create({ Type = "Spell", ID = 213658, isTalent = true    }),
    NimbleBrew                                = Create({ Type = "Item", ID = 137648, Color = "RED", QueueForbidden = true }),    
    -- Items
    BattlePotionOfAgility                     = Create({ Type = "Potion", ID = 163223 }),    
    SuperiorBattlePotionOfAgility            = Create({ Type = "Potion", ID = 168489 }),    
}
...
local Temp                                     = {
    ...
    NimbleBrewValidLoC                        = {"ROOT", "STUN", "FEAR", "HORROR"},
    NimbleBrewMissedLoC                        = {"DISARM", "INCAPACITATE", "DISORIENT", "FREEZE", "SILENCE", "POSSESS", "SAP", "CYCLONE", "BANISH", "PACIFYSILENCE", "POLYMORPH", "SLEEP", "SHACKLE_UNDEAD"},
    NiuzaosEssenceMissedLoC                    = LoC.GetExtra.Gnome.Missed,
}
...
local function ShouldTaunt(unit)
    if A.Zone ~= "none" then                 
        if AdditionalTaunt[3] and BlackOxStatue() > 0 and A.ProvokeSummonBlackOxStatue:IsReady(unit, true) and (not A.IsInPvP or not A.Admonishment:IsSpellLearned()) and MultiUnits:GetByRangeTaunting(20, 2) >= 2 then
...
local function Interrupts(unit)
    ...
    if useCC and A.LegSweep:IsReady(unit, true) and Unit(unit):GetRange() <= 5 and Unit(unit):IsCastingRemains() > GetCurrentGCD() + 0.1 and A.LegSweep:AbsentImun(unit, Temp.TotalAndPhysAndCCAndStun, true) and Unit(unit):IsControlAble("stun", 0) then
        return A.LegSweep     
    end     
end 
...
local function SharedMouseover()
    ...
    if IsUnitFriendly("mouseover") then                 
        ...
        if A.TigersLust:IsReady(unit) and A.TigersLust:AbsentImun(unit) and IsSchoolFree() and Unit(unit):HasDeBuffs("Rooted") > GetGCD() then 
            return A.TigersLust
        end 
        ...
        if GetToggle(2, "AoE") and A.ChiBurst:IsReady(unit, true) and A.ChiBurst:AbsentImun(unit) and IsSchoolFree() and Unit(unit):GetRange() <= 40 and Player:IsStaying() and A.ChiBurst:PredictHeal("ChiBurst", unit) and 
        (
            not A.IsInPvP or 
        ...
        if GetToggle(2, "AoE") and A.ChiWave:IsReady(unit, true) and A.ChiWave:AbsentImun(unit) and IsSchoolFree() and Unit(unit):GetRange() <= 25 and A.ChiWave:PredictHeal("ChiWave", unit) and
        (
            not A.IsInPvP or 
    ...
    if IsUnitEnemy("mouseover") then 
        ...
        if     GetToggle(2, "AoE") and A.ChiBurst:IsReady(unit, true) and A.ChiBurst:AbsentImun(unit) and IsSchoolFree() and Unit(unit):GetRange() <= 40 and Player:IsStaying() and MultiUnits:GetByRange(40, 3) >= 3 and
        (
            not A.IsInPvP or 
            not EnemyTeam("HEALER"):IsBreakAble(40)                
        )
        ...
        if     GetToggle(2, "AoE") and A.ChiWave:IsReady(unit, true) and A.ChiWave:AbsentImun(unit) and IsSchoolFree() and Unit(unit):GetRange() <= 25 and MultiUnits:GetByRange(25, 3) >= 3 and
        (
            not A.IsInPvP or 
            not EnemyTeam("HEALER"):IsBreakAble(25)                
        )
...
-- [3] Single Rotation
A[3] = function(icon)
    if not AdditionalTaunt then 
        AdditionalTaunt = GetToggle(2, "AdditionalTaunt")
    end 
    ...
    if A.IsInPvP and A.TigersLust:IsReady(player) and IsSchoolFree() and Unit(player, 5):HasFlags() and not IsUnitFriendly("target") and not IsUnitFriendly("mouseover") then 
        if Unit(player):HasDeBuffs("Rooted") > GetGCD() then 
    ...
    if IsUnitEnemy("target") then 
        ...
        if Unit(player):CombatTime() == 0 then         
            ...
            if GetToggle(2, "AoE") and A.ChiBurst:IsReady(unit, true) and A.ChiBurst:AbsentImun(unit, Temp.TotalAndMag) and Unit(unit):GetRange() <= 40 and Player:IsStaying() and Pull < 2 then 
                return A.ChiBurst:Show(icon)
            end 
            
            if GetToggle(2, "AoE") and A.RushingJadeWind:IsReady(unit, true) and A.RushingJadeWind:AbsentImun(unit, Temp.TotalAndMag) and IsSchoolFree() and Unit(unit):GetRange() <= 40 and Pull < 3 and 
            (
                not A.IsInPvP or 
                (
        ...
        -- Taunting 
        if AdditionalTaunt[2] and A.SummonBlackOxStatue:IsReady(unit, true) and (not A.IsInPvP or not A.Admonishment:IsSpellLearned()) and MultiUnits:GetByRangeTaunting(20, 2) >= 2 then
        ...
        -- Burstings
        if BurstIsON(unit) and inMelee and A.AbsentImun(nil, unit, Temp.TotalAndPhys) and LoC:IsMissed("DISARM") then 
        ...
        if A.InvokeNiuzaotheBlackOx:IsReady(unit) and A.InvokeNiuzaotheBlackOx:AbsentImun(unit, Temp.TotalAndPhys) and IsSchoolFree() and (BurstIsON(unit) or AdditionalTaunt[1] and not Unit(unit):IsPlayer() and not Unit(unit):IsTotem() and Unit(unit):CombatTime() > 0 and not Unit("targettarget"):IsTank()) then 
        ...
        if A.BlackOxBrew:IsReady(player) then             
            if A.IronskinBrew:GetSpellChargesFrac() <= 0.5 then 
                ...
                if Player:Energy() + (Player:EnergyRegen() * A.KegSmash:GetCooldown()) < 40 and Unit(player):HasBuffs(A.BlackoutComboBuff.ID, true) == 0 and A.KegSmash:GetCooldown() > 0 then
                    ...
                    if A.IronskinBrew:GetSpellChargesFrac() >= 2 and GetStaggerPercent() >= 1 then 
                    ...
                    if A.IronskinBrew:GetSpellChargesFrac() >= 1 then 
                        ...
                        if A.PurifyingBrew:IsReady(player) and GetStaggerPercent() >= 1 then 
                            return A.PurifyingBrew:Show(icon)
                        end
                    end 
        ...
        if  GetToggle(2, "AoE") and A.KegSmash:IsReady(unit) and A.KegSmash:AbsentImun(unit, Temp.TotalAndPhys) and MultiUnits:GetByRange(8, 2) >= 2 and 
        (
            not A.IsInPvP or 
        ...
        -- keg_smash
        if     GetToggle(2, "AoE") and A.KegSmash:IsReady(unit) and A.KegSmash:AbsentImun(unit, Temp.TotalAndPhys) and 
        (
            not A.IsInPvP or 
            not EnemyTeam("HEALER"):IsBreakAble(8)
        ) 
        ...
        if     GetToggle(2, "AoE") and A.RushingJadeWind:IsReady(unit, true) and A.RushingJadeWind:AbsentImun(unit, Temp.TotalAndMag) and IsSchoolFree() and Unit(unit):GetRange() <= 8 and Unit(player):HasBuffs(A.RushingJadeWind.ID, true) == 0 and 
        (
            not A.IsInPvP or 
            not EnemyTeam("HEALER"):IsBreakAble(8)                
        )
        ...
        if     GetToggle(2, "AoE") and A.BreathofFire:IsReady(unit, true) and A.BreathofFire:AbsentImun(unit, Temp.TotalAndMag) and LoC:IsMissed("SILENCE") and Unit(unit):GetRange() <= 12 and (Unit(player):HasBuffs(A.BlackoutComboBuff.ID, true) == 0 or (Unit(player):HasBuffs("BurstHaste") == 0 or Unit(unit):PT(A.BreathofFireDotDebuff.ID, "debuff"))) and 
        (
            not A.IsInPvP or 
        ...
        -- chi_burst
        if     GetToggle(2, "AoE") and A.ChiBurst:IsReady(unit, true) and A.ChiBurst:AbsentImun(unit, Temp.TotalAndMag) and IsSchoolFree() and Unit(unit):GetRange() <= 40 and Player:IsStaying() and 
        (
            not A.IsInPvP or 
            not EnemyTeam("HEALER"):IsBreakAble(40)                
        )
        ...
        -- chi_wave
        if     GetToggle(2, "AoE") and A.ChiWave:IsReady(unit, true) and A.ChiWave:AbsentImun(unit, Temp.TotalAndMag) and IsSchoolFree() and Unit(unit):GetRange() <= 25 and MultiUnits:GetByRange(25, 2) >= 2 and 
        (
            not A.IsInPvP or 
            not EnemyTeam("HEALER"):IsBreakAble(25)                
        )
        ...
        if A.TigerPalm:IsReady(unit) and A.TigerPalm:AbsentImun(unit, Temp.TotalAndMag) and (not A.BlackoutComboBuff:IsSpellLearned() and A.KegSmash:GetCooldown() > GetGCD() and (Player:Energy() + (Player:EnergyRegen() * (A.KegSmash:GetCooldown() + GetGCD()))) >= 65) then 
            return A.TigerPalm:Show(icon)
        end 
        ...
        -- rushing_jade_wind
        if     GetToggle(2, "AoE") and A.RushingJadeWind:IsReady(unit, true) and A.RushingJadeWind:AbsentImun(unit, Temp.TotalAndMag) and IsSchoolFree() and MultiUnits:GetByRange(8, 2) >= 1 and 
        (
            not A.IsInPvP or 
            not EnemyTeam("HEALER"):IsBreakAble(8)                
        )
        ...
        -- Custom APL 
        -- Detox 
        if A.Detox:IsReady(player) and AuraIsValid(player, "UseDispel", "Dispel") and IsSchoolFree() then 
            return A.Detox:Show(icon)
        end 
    ...
    -- Misc 
    if IsUnitEnemy("mouseover") and A.CracklingJadeLightning:IsReady("mouseover") and A.CracklingJadeLightning:AbsentImun("mouseover", Temp.TotalAndMag) and Player:IsStaying() and Unit("mouseover"):CombatTime() > 0 then
        return A.CracklingJadeLightning:Show(icon)
    end 
    
    if IsUnitEnemy("target") and Unit("target"):GetRange() > 8 and A.CracklingJadeLightning:IsReady("target") and A.CracklingJadeLightning:AbsentImun("target", Temp.TotalAndMag) and Player:IsStaying() then
        return A.CracklingJadeLightning:Show(icon)
    end 
...
A[5] = function(icon)
    if A.IsInPvP then 
        if A.NimbleBrew:IsReady(player) and A.NimbleBrew:GetCount() > 0 and A.NimbleBrew:GetItemCooldown() == 0 and LoC:IsValid(Temp.NimbleBrewValidLoC, Temp.NimbleBrewMissedLoC) then 
            return A.NimbleBrew:Show(icon)             
        end 
        
        -- Niuzao's Essence (PvP talent) which removes slow effects 
        if IsSpellLearned(232876) and A.PurifyingBrew:IsReadyP(player) and LoC:IsMissed(Temp.NiuzaosEssenceMissedLoC) then 
...
A[6] = function(icon)
    ...
    if A.IsInPvP and GetToggle(1, "AutoTarget") and IsUnitEnemy("target") and not A.AbsentImun(nil, "target", Temp.TotalAndPhys) and MultiUnits:GetByRangeInCombat(8, 2) >= 2 then 
