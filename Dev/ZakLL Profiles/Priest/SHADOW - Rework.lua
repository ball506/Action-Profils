...
A[3] = function(icon)
	...
	local function EnemyDot(unitID) 
		if A.ShadowWordPain:IsReady(unitID) and IsSchoolShadowUP() and A.ShadowWordPain:AbsentImun(unitID, Temp.AttackTypes) and ((EvaluateCycleShadowWordPain(unitID) and Unit(unitID):TimeToDie() > 4 and not A.Misery:IsSpellLearned()) or (inMoving and (not inVoidform or A.VoidBolt:GetCooldown() >= A.GetGCD() + A.GetCurrentGCD() + A.GetPing()))) then
			return A.ShadowWordPain:Show(icon)
		end
			
		if A.VampiricTouch:IsReady(unitID) and IsSchoolShadowUP() and A.VampiricTouch:AbsentImun(unitID, Temp.AttackTypes) and (EvaluateCycleVampiricTouch(unitID) or (A.Misery:IsSpellLearned() and EvaluateCycleShadowWordPain(unitID))) then
	...
	local function Enemy(unitID)		
		...
		--Void Eruption
		if unitID ~= "targettarget" and A.VoidEruption:IsReady(unitID, nil, nil, A.GetToggle(2, "ByPassSpells")) and IsSchoolShadowUP() and A.VoidEruption:AbsentImun(unitID, Temp.AttackTypes) and Player:Insanity() >= insanityThreshold and (canCast or (A.IsInPvP and A.VoidOrigins:IsSpellLearned())) and not A.FocusedAzeriteBeam:IsSpellInCasting() and not inVoidform and not A.FocusedAzeriteBeam:IsSpellInCasting() then
			return A.VoidEruption:Show(icon)
		end
		--Void Bolt
		if unitID ~= "targettarget" and ((A.GetToggle(2, "SpellsTiming") and inVoidform and A.VoidBolt:GetCooldown() <= A.GetGCD() + A.GetCurrentGCD() + A.GetPing() + (TMW.UPD_INTV or 0) + ACTION_CONST_CACHE_DEFAULT_TIMER) or (A.VoidBolt:IsReady(unitID, nil, nil, A.GetToggle(2, "ByPassSpells")) and inVoidform)) and IsSchoolShadowUP() and A.VoidBolt:AbsentImun(unitID, Temp.AttackTypes) and not A.FocusedAzeriteBeam:IsSpellInCasting() then
			return A.VoidBolt:Show(icon)
		end
		
		-- CD's
		if unit ~= "mouseover" and A.BurstIsON(unitID) and A.AbsentImun(nil, unitID, Temp.AttackTypes) then 
		
			if A.MemoryofLucidDreams:AutoHeartOfAzerothP(unitID) and ((Unit("player"):HasBuffsStacks(A.VoidformBuff.ID, true) > 20 and Player:Insanity() <= 50) or Unit("player"):HasBuffsStacks(A.VoidformBuff.ID, true) > (26 + 7 * Unit("player"):HasBuffs("BurstHaste")) or (InsanityDrain() *((A.GetGCD() * 2) + A.MindBlast:GetSpellCastTime())) > Player:Insanity()) then
			...
			--if A.MemoryofLucidDreams:AutoHeartOfAzerothP(unitID) and (Unit("player"):HasBuffsStacks(A.VoidformBuff.ID, true) > 20 and Player:Insanity() <= 50)     then
			--	return A.MemoryofLucidDreams:Show(icon)
			--end
		...
		if not inAoE or MultiUnits:GetActiveEnemies() < 2 then 		
			...
			if A.MindBlast:IsReady(unitID, nil, nil, A.GetToggle(2, "ByPassSpells")) and IsSchoolShadowUP() and A.MindBlast:AbsentImun(unitID, Temp.AttackTypes) and (Unit(unitID):HasDeBuffs(A.ShadowWordPain.ID) > 0 and Unit(unitID):HasDeBuffs(A.VampiricTouchDebuff.ID) > 0 and canCast or Unit("player"):HasBuffs(A.ShadowyInsightBuff.ID) > 0) and
				(isSpellsTiming or (not isSpellsTiming and A.VoidBolt:GetCooldown() >= A.GetGCD() + A.GetCurrentGCD() or not inVoidform)) and not A.FocusedAzeriteBeam:IsSpellInCasting() then
			...
			if A.MindFlay:IsReady(unitID) and A.MindFlay:AbsentImun(unitID, Temp.AttackTypes) and IsSchoolShadowUP() and canCast and (A.MindBlast:GetCooldown() >= A.GetGCD() or (not inVoidform or A.VoidBolt:GetCooldown() >= A.GetGCD())) then 
				return A.MindFlay:Show(icon)		 
			end					
		end
	end
