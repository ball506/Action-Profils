...
--	if Unit("player"):HealthPercent() < 50 then
	...
	local function Enemy(unitID)
		...
		if Temp.OpenerRotation and Unit(unitID):GetRange() <= openerDistance and canCast then 
			...
			if A.ThrashCat:IsReady(unitID, true) and A.ThrashCat:AbsentImun(unitID, Temp.AttackTypes) and canCast and InMelee(unitID) and isWildFleshRending and Unit(unitID):HasDeBuffs(A.ThrashCatDebuff.ID) == 0 then
