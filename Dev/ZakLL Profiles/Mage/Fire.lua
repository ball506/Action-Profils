...
A[3] = function(icon)
	...
	local function Enemy(unitID)		
		...
		if A.RuneofPower:IsReady(player) and (A.Firestarter:IsSpellLearned() and Unit(unitID):TimeToDieX(90) > A.RuneofPower:GetSpellChargesFullRechargeTime() or A.Combustion:GetCooldown() > Temp.CombustionRopCutOff
		and Unit(player):HasBuffs(A.CombustionBuff.ID) == 0 or Unit(unitID):TimeToDie() < A.Combustion:GetCooldown() and Unit(player):HasBuffs(A.CombustionBuff.ID) == 0 or disableCombustion) and not inMoving then
			return A.RuneofPower:Show(icon)
		end
		
		-- Combustion Phase
		if not disableCombustion and (A.RuneofPower:IsSpellLearned() and A.Combustion:GetCooldown() <= A.RuneofPower:GetSpellCastTime() or A.Combustion:GetCooldown() == 0) and not FirestarterActive(unitID) or
		Unit(player):HasBuffs(A.CombustionBuff.ID) > 0 then 
			...
			if A.MemoryofLucidDreams:AutoHeartOfAzerothP(unitID) and (not A.Meteor:IsSpellLearned() or A.Meteor:GetCooldown() == 0) then
			...
			Unit(player):HasBuffs(A.HotStreakBuff.ID) == 0)) then
				return A.FireBlast:Show(icon)
			end
		
			if A.RuneofPower:IsReady(player) and Unit(player):HasBuffs(A.CombustionBuff.ID) == 0 and not inMoving  then
				return A.RuneofPower:Show(icon)
			end
			...
			(A.Combustion:GetCooldown() == 0 or Unit(player):HasBuffs(A.CombustionBuff.ID) > 0) and not A.RuneofPower:IsSpellLearned() and not A.Pyroblast:IsSpellInFlight() and not A.Fireball:IsSpellInFlight())) then
				return A.FireBlast:Show(icon)
			end
			...
			if A.Combustion:IsReady(player) and (((A.Meteor:IsSpellInFlight() or A.LastPlayerCastID == A.Meteor.ID or A.Meteor:GetCooldown() >= 40) or not A.Meteor:IsSpellLearned()) and 
			(Unit(player):HasBuffs(A.RuneofPowerBuff.ID) > 0 or not A.RuneofPower:IsSpellLearned())) then
			...
			MultiUnits:GetActiveEnemies() > 6) and Unit(player):HasBuffs(A.HotStreakBuff.ID) > 0 and A.BlasterMaster:GetAzeriteRank() == 0) and not inMoving  then
				return A.Flamestrike:Show(icon)
			end			
			
			if A.Pyroblast:IsReady(unitID) and IsSchoolFireUP() and A.Pyroblast:AbsentImun(unitID, Temp.AttackTypes) and (Unit(player):HasBuffs(A.PyroclasmBuff.ID) > 0 and 
			Unit(player):HasBuffs(A.CombustionBuff.ID) > A.Pyroblast:GetSpellCastTime()) then
				return A.Pyroblast:Show(icon)
			end
			
			if A.Pyroblast:IsReady(unitID) and IsSchoolFireUP() and A.Pyroblast:AbsentImun(unitID, Temp.AttackTypes) and Unit(player):HasBuffs(A.HotStreakBuff.ID) > 0 then
				return A.Pyroblast:Show(icon)
			end
			
			if A.Pyroblast:IsReady(unitID) and IsSchoolFireUP() and A.Pyroblast:AbsentImun(unitID, Temp.AttackTypes) and (A.LastPlayerCastID == A.Scorch.ID and Unit(player):HasBuffs(A.HeatingUpBuff.ID) > 0) then
				return A.Pyroblast:Show(icon)
			end
			
			if A.PhoenixFlames:IsReady(unitID) and IsSchoolFireUP() and A.PhoenixFlames:AbsentImun(unitID, Temp.AttackTypes) then
			...
			Unit(player):HasBuffs(A.CombustionBuff.ID) > 0 or Unit(player):HasBuffs(A.CombustionBuff.ID) == 0) then
			...
			MultiUnits:GetActiveEnemies() > 1 and inAoE) then
				return A.LivingBomb:Show(icon)
			end
			
			if A.DragonsBreath:IsReady(unitID, true) and IsSchoolFireUP() and A.DragonsBreath:AbsentImun(unitID, Temp.AttackTypes) and (Unit(player):HasBuffs(A.CombustionBuff.ID) < A.GetGCD() + A.GetCurrentGCD() and
			Unit(player):HasBuffs(A.CombustionBuff.ID) > 0) then
		...
		if A.FireBlast:IsReadyByPassCastGCD(unitID, nil, nil, true) and IsSchoolFireUP() and A.FireBlast:AbsentImun(unitID, Temp.AttackTypes) and (((A.MemoryofLucidDreams:IsReady(player) or 
		A.MemoryofLucidDreams:GetCooldown() > 0) and A.BlasterMaster:GetAzeriteRank() >= 1) and A.FireBlast:GetSpellCharges() == A.FireBlast:GetSpellChargesMax() and Unit(player):HasBuffs(A.HotStreakBuff.ID) == 0 and not
		(Unit(player):HasBuffs(A.HeatingUpBuff.ID) > 0 and (Unit(player):HasBuffs(A.CombustionBuff.ID) > 0 and (A.Fireball:IsSpellInFlight() or A.Pyroblast:IsSpellInFlight() or A.Scorch:IsSpellInCasting()) or 
		Unit(unitID):HealthPercent() <= 30 and A.Scorch:IsSpellInCasting())) and not (Unit(player):HasBuffs(A.HeatingUpBuff.ID) == 0 or Unit(player):HasBuffs(A.HotStreakBuff.ID) == 0 and 
		Unit(player):HasBuffs(A.CombustionBuff.ID) == 0 and (A.Fireball:IsSpellInFlight() or A.Pyroblast:IsSpellInFlight()))) then
			return A.FireBlast:Show(icon)
		end
		
		if A.FireBlast:IsReadyByPassCastGCD(unitID, nil, nil, true) and IsSchoolFireUP() and A.FireBlast:AbsentImun(unitID, Temp.AttackTypes) and (FirestarterActive(unitID) and A.FireBlast:GetSpellCharges() >= 1 and
		(not Temp.FireblastPooling or Unit(player):HasBuffs(A.RuneofPowerBuff.ID) > 0) and (A.BlasterMaster:GetAzeriteRank() == 0 or Unit(player):HasBuffs(A.BlasterMasterBuff.ID) < 0.5) and 
		(not A.Fireball:IsSpellInCasting() or not A.Pyroblast:IsSpellInFlight() and Unit(player):HasBuffs(A.HeatingUpBuff.ID) > 0 or A.Fireball:IsSpellInCasting() and Unit(player):HasBuffs(A.HotStreakBuff.ID) == 0 or
		A.Pyroblast:IsSpellInFlight() and Unit(player):HasBuffs(A.HeatingUpBuff.ID) == 0 and Unit(player):HasBuffs(A.HotStreakBuff.ID) == 0)) then
			return A.FireBlast:Show(icon)
		end
		
		
		-- RoP Phase
		if Unit(player):HasBuffs(A.RuneofPowerBuff.ID) > 0 and Unit(player):HasBuffs(A.CombustionBuff.ID) == 0 then
			...
			if A.Pyroblast:IsReady(unitID) and IsSchoolFireUP() and A.Pyroblast:AbsentImun(unitID, Temp.AttackTypes) and (A.LastPlayerCastID == A.Scorch.ID and Unit(player):HasBuffs(A.HeatingUpBuff.ID) > 0 and
			A.SearingTouch:IsSpellLearned() and Unit(unitID):HealthPercent() <= 30 and (not A.FlamePatch:IsSpellLearned() or (MultiUnits:GetActiveEnemies() == 1 or not inAoE))) then
				return A.Pyroblast:Show(icon)
			end
		...
		disableCombustion or FirestarterActive(unitID)) and (A.RuneofPower:GetCooldown() < Unit(unitID):TimeToDie() or A.RuneofPower:GetSpellCharges() > 0) or not disableCombustion and
		A.Combustion:GetCooldown() < A.FireBlast:GetSpellChargesFullRechargeTime() * A.BlasterMaster:GetAzeriteRank() and not FirestarterActive(unitID) and A.Combustion:GetCooldown() <
		Unit(unitID):TimeToDie() or FirestarterActive(unitID) and Unit(unitID):TimeToDieX(90) < A.FireBlast:GetSpellChargesFullRechargeTime() * A.BlasterMaster:GetAzeriteRank() then
		...
		if A.Pyroblast:IsReady(unitID) and IsSchoolFireUP() and A.Pyroblast:AbsentImun(unitID, Temp.AttackTypes) and (Unit(player):HasBuffs(A.HotStreakBuff.ID) > 0 and 
		Unit(player):HasBuffs(A.HotStreakBuff.ID) < A.Pyroblast:ExecuteTime()) then
			return A.Pyroblast:Show(icon)
		end
		
		if A.Pyroblast:IsReady(unitID) and IsSchoolFireUP() and A.Pyroblast:AbsentImun(unitID, Temp.AttackTypes) and (Unit(player):HasBuffs(A.HotStreakBuff.ID) > 0 and 
		(A.LastPlayerCastID == A.Fireball.ID or FirestarterActive(unitID) or A.Pyroblast:IsSpellInFlight())) then
			return A.Pyroblast:Show(icon)
		end
		...
		if A.FireBlast:IsReadyByPassCastGCD(unitID, nil, nil, true) and IsSchoolFireUP() and A.FireBlast:AbsentImun(unitID, Temp.AttackTypes) and (((A.Combustion:GetCooldown() > 0 or
		disableCombustion) and Unit(player):HasBuffs(A.RuneofPowerBuff.ID) == 0 and not FirestarterActive(unitID)) and not A.Kindling:IsSpellLearned() and not Temp.FireblastPooling and
		(((A.Fireball:IsSpellInCasting() or A.Pyroblast:IsSpellInCasting()) and (Unit(player):HasBuffs(A.HeatingUpBuff.ID) > 0)) or (A.SearingTouch:IsSpellLearned() and 
		Unit(unitID):HealthPercent() <= 30 and (Unit(player):HasBuffs(A.HeatingUpBuff.ID) > 0 and not A.Scorch:IsSpellInCasting() or Unit(player):HasBuffs(A.HotStreakBuff.ID) == 0 or 
		Unit(player):HasBuffs(A.HeatingUpBuff.ID) == 0 and A.Scorch:IsSpellInCasting() and not A.Pyroblast:IsSpellInFlight() and not A.Fireball:IsSpellInFlight())))) then
			return A.FireBlast:Show(icon)
		end
		...
		if A.Pyroblast:IsReady(unitID) and IsSchoolFireUP() and A.Pyroblast:AbsentImun(unitID, Temp.AttackTypes) and (A.LastPlayerCastID == A.Scorch.ID and 
		Unit(player):HasBuffs(A.HeatingUpBuff.ID) > 0 and A.SearingTouch:IsSpellLearned() and Unit(unitID):HealthPercent() <= 30 and ((A.FlamePatch:IsSpellLearned() and
		(inAoE and MultiUnits:GetActiveEnemies() == 1 or not inAoE) and not FirestarterActive(unitID)) or ((inAoE and MultiUnits:GetActiveEnemies() < 4 or not inAoE) and 
		not A.FlamePatch:IsSpellLearned()))) then
			return A.Pyroblast:Show(icon)
		end
		...
		if A.FireBlast:IsReadyByPassCastGCD(unitID, nil, nil, true) and IsSchoolFireUP() and A.FireBlast:AbsentImun(unitID, Temp.AttackTypes) and (not Temp.FireblastPooling and (A.FlamePatch:IsSpellLearned() and
		MultiUnits:GetActiveEnemies() > 2 or MultiUnits:GetActiveEnemies() > 9) and inAoE and ((A.Combustion:GetCooldown() > 0 or disableCombustion) and not FirestarterActive(unitID)) and 
		Unit(player):HasBuffs(A.HotStreakBuff.ID) == 0 and (A.BlasterMaster:GetAzeriteRank() == 0 or Unit(player):HasBuffs(A.BlasterMasterBuff.ID) < 0.5)) then
			return A.FireBlast:Show(icon)
		end
		...
		if A.Scorch:IsReady(unitID) and IsSchoolFireUP() and A.Scorch:AbsentImun(unitID, Temp.AttackTypes) and inMoving then
			return A.Scorch:Show(icon)
		end
