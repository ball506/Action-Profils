...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Obliteration(unit)
            ...
            if A.Frostscythe:IsReady(unit) and ((Unit("player"):HasBuffsStacks(A.KillingMachineBuff.ID, true) or (Unit("player"):HasBuffs(A.KillingMachineBuff.ID, true) and (Unit("player"):PrevGCDP(1, A.FrostStrike) or Unit("player"):PrevGCDP(1, A.HowlingBlast) or Unit("player"):PrevGCDP(1, A.GlacialAdvance)))) and MultiUnits:GetByRangeInCombat(8, 5, 10) >= 2) then
                return A.Frostscythe:Show(icon)
            end
            ...
            if A.Obliterate:IsReady(unit) and (Unit("player"):HasBuffsStacks(A.KillingMachineBuff.ID, true) or (Unit("player"):HasBuffs(A.KillingMachineBuff.ID, true) and (Unit("player"):PrevGCDP(1, A.FrostStrike) or Unit("player"):PrevGCDP(1, A.HowlingBlast) or Unit("player"):PrevGCDP(1, A.GlacialAdvance)))) then
                return A.Obliterate:Show(icon)
            end
