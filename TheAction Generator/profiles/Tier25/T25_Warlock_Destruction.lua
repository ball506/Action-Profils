...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Cds(unit)
            ...
            if A.WorldveinResonance:AutoHeartOfAzerothP(unit, true) and Action.GetToggle(1, "HeartOfAzeroth") and (bool(pet.infernal.active) and (pet.infernal.remains < 18.5 or pet.infernal.remains < 20 and Player:SoulShardsP >= 3.6 or not A.GrimoireofSupremacy:IsSpellLearned())) then
            ...
            if A.WorldveinResonance:AutoHeartOfAzerothP(unit, true) and Action.GetToggle(1, "HeartOfAzeroth") and (A.SummonInfernal:GetCooldown() >= 60 - 12 and not bool(pet.infernal.active)) then
                return A.WorldveinResonance:Show(icon)
            end
            -- ripple_in_space
...
if not inCombat and Unit(unit):IsExists() and Action.GetToggle(1, "DBM") and unit ~= "mouseover" and not Unit(unit):IsTotem() then
  local ShouldReturn = Precombat(unit); if ShouldReturn then return ShouldReturn; end
        ...
        if inCombat and Unit(unit):IsExists() and not Unit(unit):IsTotem() then
            ...
            if A.Shadowburn:IsReady(unit) and (Player:SoulShardsP < 2 and (not bool(VarPoolSoulShards) or A.Shadowburn:GetSpellCharges() > 1)) then
                return A.Shadowburn:Show(icon)
            end
