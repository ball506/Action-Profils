...
Action.Listener:Add("TASTE_EVENT_MAGE_BURN_RESET", "PLAYER_REGEN_DISABLED", 	function()	BurnPhase:Reset()			end)
...
A[3] = function(icon, isMulti)
	...
	local PresenceOfMindMax = PresenceOfMindMax()
	local ArcaneMissilesProcMax = ArcaneMissilesProcMax()
	local ManaSaveToBurn = A.GetToggle(2, "ManaSaveToBurn")
    ...
    local function EnemyRotation(unit)
        ...
        --Precombat
		--print(Player:ManaPercentage())
		--print(ManaSaveToBurn)
        ...
        local function Precombat(unit)
            ...
            -- arcane_familiar
            if A.ArcaneFamiliar:IsReady(unit) and Unit("player"):HasBuffs(A.ArcaneFamiliarBuff.ID, true) == 0 then
        ...
        local function Burn(unit)
            ...
            if (not BurnPhase:On()) and Player:ManaPercentage() > ManaSaveToBurn then
                BurnPhase:Start()
            end
            ...
            if A.ArcaneBlast:IsReady(unit) and Unit("player"):HasBuffs(A.RuleofThreesBuff.ID, true) > 0 and A.Overpowered:IsSpellLearned() and (MultiUnits:GetByRange(10) < 3 or not A.GetToggle(2, "AoE")) then
                return A.ArcaneBlast:Show(icon)
            end
            ...
            if A.BagofTricks:IsReady(unit) and A.BurstIsON(unit) and (Unit("player"):HasBuffs(A.ArcanePowerBuff.ID, true) == 0) then
            ...
            if A.ArcaneOrb:IsReady(unit) and (Player:ArcaneChargesP() == 0 or (MultiUnits:GetByRange(10) < 3 or (MultiUnits:GetByRange(10) < 2 and A.Resonance:IsSpellLearned()))) then
                return A.ArcaneOrb:Show(icon)
            end
            -- arcane_barrage,if=active_enemies>=3&(buff.arcane_charge.stack=buff.arcane_charge.max_stack)
            if A.ArcaneBarrage:IsReady(unit) and (MultiUnits:GetByRange(10) >= 3 and (Player:ArcaneChargesP() == Player:ArcaneChargesMax())) then
                return A.ArcaneBarrage:Show(icon)
            end
            -- arcane_explosion,if=active_enemies>=3
            if A.ArcaneExplosion:IsReady(unit) and (MultiUnits:GetByRange(10) > 2) and A.GetToggle(2, "AoE") then
            ...
            if A.ArcaneMissiles:IsReady(unit) and (Unit("player"):HasBuffsStacks(A.ClearcastingBuff.ID, true) > 0 and MultiUnits:GetByRange(10) < 3 and (A.Amplification:IsSpellLearned() or (not A.Overpowered:IsSpellLearned() and A.ArcanePummeling:GetAzeriteRank() >= 2) or Unit("player"):HasBuffs(A.ArcanePowerBuff.ID, true) == 0)) then
                return A.ArcaneMissiles:Show(icon)
            end
            -- arcane_blast,if=active_enemies<3
            if A.ArcaneBlast:IsReady(unit) and (MultiUnits:GetByRange(10) < 3 or not A.GetToggle(2, "AoE")) then
                return A.ArcaneBlast:Show(icon)
            end
            ...
            if A.Evocation:IsReady("player") then
			    -- if mana < 85 use Evocation
			    if Player:ManaPercentageP() < 85 then
                    return A.Evocation:Show(icon)
				-- else cut evocation
				else
				    if Unit("player"):IsCasting(A.Evocation) and Player:ManaPercentageP() >= 85 then
        ...
        local function Conserve(unit)
            ...
            if A.ArcaneOrb:IsReady(unit) and (Player:ArcaneChargesP() <= 2 and (A.ArcanePower:GetCooldown() > 10 or MultiUnits:GetByRange(10) <= 2)) then
                return A.ArcaneOrb:Show(icon)
            end
            ...
            if A.RuneofPower:IsReady("player") and (Player:ArcaneChargesP() == Player:ArcaneChargesMax() and (A.RuneofPower:GetSpellChargesFullRechargeTime() <= A.RuneofPower:GetSpellCastTime() or A.RuneofPower:GetSpellChargesFullRechargeTime() <= A.ArcanePower:GetCooldown() or Unit(unit):TimeToDie() <= A.ArcanePower:GetCooldown())) then
                return A.RuneofPower:Show(icon)
            end
            -- arcane_missiles,if=mana.pct<=95&buff.clearcasting.react&active_enemies<3,chain=1
            if A.ArcaneMissiles:IsReady(unit) and (Player:ManaPercentageP() <= 95 and Unit("player"):HasBuffsStacks(A.ClearcastingBuff.ID, true) > 0 and MultiUnits:GetByRange(10) < 3) then
                return A.ArcaneMissiles:Show(icon)
            end
            ...
            if A.ArcaneExplosion:IsReady(unit) and A.GetToggle(2, "AoE") and (MultiUnits:GetByRange(10) > 2 and (Player:ManaPercentageP() >= VarConserveMana or Player:ArcaneChargesP() == 3)) then
                return A.ArcaneExplosion:Show(icon)
            end
            -- arcane_blast
            if A.ArcaneBlast:IsReady(unit) then
        ...
        local function Movement(unit)
            ...
            -- arcane_missiles
            if A.ArcaneExplosion:IsReady(unit) and A.GetToggle(2, "AoE") and MultiUnits:GetByRange(10) > 2 then
                return A.ArcaneExplosion:Show(icon)
            end
            -- arcane_orb
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.AzsharasFontofPower:IsReady("player") and (Unit("player"):HasBuffs(A.RuneofPowerBuff.ID, true) == 0 and Unit("player"):HasBuffs(A.ArcanePowerBuff.ID, true) == 0 and (A.ArcanePower:GetCooldown() <= 4 + 10 * VarFontDoubleOnUse and A.Evocation:GetCooldown() <= VarAverageBurnLength + 4 + 10 * VarFontDoubleOnUse or Unit(unit):TimeToDie() < A.ArcanePower:GetCooldown())) then
                return A.AzsharasFontofPower:Show(icon)
            end
            -- call_action_list,name=burn,if=burn_phase|target.time_to_die<variable.average_burn_length
            if Burn(unit) and (BurnPhase:On() or Unit(unit):TimeToDie() < VarAverageBurnLength) then
                return true
            end
            ...
            if Conserve(unit) and (not BurnPhase:On()) then
                return true
            end
            -- call_action_list,name=movement
            if isMoving and Movement(unit) then
                return true
            end
        end
    end
