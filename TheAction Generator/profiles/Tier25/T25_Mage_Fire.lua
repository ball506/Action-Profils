...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function ActiveTalents(unit)
            ...
            if A.Meteor:IsReady(unit) and (Unit("player"):HasBuffs(A.RuneofPowerBuff.ID, true)) and (S.Firestarter:ActiveRemains() > A.Meteor:BaseDuration() or not S.Firestarter:ActiveStatus()) or A.RuneofPower:GetCooldown() > Unit(unit):TimeToDie() and A.RuneofPower:GetSpellCharges() < 1 or (A.Meteor:BaseDuration() < VarTimeToCombustion or VarTimeToCombustion <= 0 or VarDisableCombustion) and not A.RuneofPower:IsSpellLearned() and (A.Meteor:BaseDuration() < S.Firestarter:ActiveRemains() or not A.Firestarter:IsSpellLearned() or not S.Firestarter:ActiveStatus())) then
        ...
        local function CombustionPhase(unit)
            ...
            if A.RuneofPower:IsReady(unit) and (Unit("player"):HasBuffsDown(A.CombustionBuff.ID, true))) then
            ...
            if A.Scorch:IsReady(unit) and (Unit("player"):HasBuffs(A.CombustionBuff.ID, true)) > A.Scorch:GetSpellCastTime() and Unit("player"):HasBuffs(A.CombustionBuff.ID, true)) or Unit("player"):HasBuffsDown(A.CombustionBuff.ID, true))) then
        ...
        local function ItemsHighPriority(unit)
            ...
            if (not VarDisableCombustion and (A.RuneofPower:IsSpellLearned() and VarTimeToCombustion <= A.RuneofPower:GetSpellCastTime() or VarTimeToCombustion <= 0) and not S.Firestarter:ActiveStatus() or Unit("player"):HasBuffs(A.CombustionBuff.ID, true))) then
        ...
        local function ItemsLowPriority(unit)
            ...
            if A.TidestormCodex:IsReady(unit) and (VarTimeToCombustion > VarOnUseCutoff or VarDisableCombustion or A.Firestarter:IsSpellLearned() and S.Firestarter:ActiveRemains() > VarOnUseCutoff) then
            ...
            if A.CyclotronicBlast:IsReady(unit) and (VarTimeToCombustion > VarOnUseCutoff or VarDisableCombustion or A.Firestarter:IsSpellLearned() and S.Firestarter:ActiveRemains() > VarOnUseCutoff) then
        ...
        local function RopPhase(unit)
            ...
            if A.FireBlast:IsReady(unit) and (not (MultiUnits:GetByRangeInCombat(40, 5, 10) >= VarHardCastFlamestrike and (Unit("player"):CombatTime() - buff.combustion.last_expire > VarDelayFlamestrike or VarDisableCombustion)) and (not S.Firestarter:ActiveStatus() and (VarTimeToCombustion > 0 or VarDisableCombustion)) and (Unit("player"):HasBuffsStacks(A.HeatingUpBuff.ID, true)) and (Unit(unit):HealthPercent() >= 30 or not A.SearingTouch:IsSpellLearned()))) then
                return A.FireBlast:Show(icon)
            end
            ...
            if A.FireBlast:IsReady(unit) and ((MultiUnits:GetByRangeInCombat(40, 5, 10) >= VarHardCastFlamestrike and (Unit("player"):CombatTime() - buff.combustion.last_expire > VarDelayFlamestrike or VarDisableCombustion)) and ((VarTimeToCombustion > 0 or VarDisableCombustion) and not S.Firestarter:ActiveStatus()) and Unit("player"):HasBuffsDown(A.HotStreakBuff.ID, true)) and (not A.BlasterMaster:GetAzeriteRank() > 0 or Unit("player"):HasBuffs(A.BlasterMasterBuff.ID, true)) < 0.5)) then
                return A.FireBlast:Show(icon)
            end
        ...
        local function StandardRotation(unit)
            ...
            if A.FireBlast:IsReady(unit) and (((VarTimeToCombustion > 0 or VarDisableCombustion) and Unit("player"):HasBuffsDown(A.RuneofPowerBuff.ID, true)) and not S.Firestarter:ActiveStatus()) and not A.Kindling:IsSpellLearned() and not VarFireBlastPooling and (((action.fireball.executing or action.pyroblast.executing) and (Unit("player"):HasBuffsStacks(A.HeatingUpBuff.ID, true)))) or (A.SearingTouch:IsSpellLearned() and Unit(unit):HealthPercent() <= 30 and (Unit("player"):HasBuffsStacks(A.HeatingUpBuff.ID, true)) and not action.scorch.executing or not Unit("player"):HasBuffsStacks(A.HotStreakBuff.ID, true)) and not Unit("player"):HasBuffsStacks(A.HeatingUpBuff.ID, true)) and action.scorch.executing and not A.Pyroblast:IsSpellInFlight() and not A.Fireball:IsSpellInFlight())))) then
                return A.FireBlast:Show(icon)
            end
            ...
            if A.FireBlast:IsReady(unit) and (A.Kindling:IsSpellLearned() and Unit("player"):HasBuffsStacks(A.HeatingUpBuff.ID, true)) and not S.Firestarter:ActiveStatus() and (VarTimeToCombustion > A.FireBlast:GetSpellChargesFullRechargeTime() + 2 + num(A.Kindling:IsSpellLearned()) or VarDisableCombustion or (not A.RuneofPower:IsSpellLearned() or A.RuneofPower:GetCooldown() > Unit(unit):TimeToDie() and A.RuneofPower:GetSpellCharges() < 1) and VarTimeToCombustion > Unit(unit):TimeToDie())) then
                return A.FireBlast:Show(icon)
            end
            ...
            if A.FireBlast:IsReady(unit) and (not VarFireBlastPooling and (MultiUnits:GetByRangeInCombat(40, 5, 10) >= VarHardCastFlamestrike and (Unit("player"):CombatTime() - buff.combustion.last_expire > VarDelayFlamestrike or VarDisableCombustion)) and ((VarTimeToCombustion > 0 or VarDisableCombustion) and not S.Firestarter:ActiveStatus()) and Unit("player"):HasBuffsDown(A.HotStreakBuff.ID, true)) and (not A.BlasterMaster:GetAzeriteRank() > 0 or Unit("player"):HasBuffs(A.BlasterMasterBuff.ID, true)) < 0.5)) then
                return A.FireBlast:Show(icon)
            end
...
if not Player:AffectingCombat() and not Player:IsCasting() then
  local ShouldReturn = Precombat(unit); if ShouldReturn then return ShouldReturn; end
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if (Azerite:EssenceHasMajor(A.MemoryofLucidDreams.ID) and Unit("player"):HasBuffsDown(A.MemoryofLucidDreamsBuff.ID, true)) and A.MemoryofLucidDreams:GetCooldown() - A.Combustion:GetCooldown() <= VarHoldCombustionThreshold) then
            ...
            if (Azerite:EssenceHasMajor(A.WorldveinResonance.ID) and Unit("player"):HasBuffsDown(A.WorldveinResonanceBuff.ID, true)) and A.WorldveinResonance:GetCooldown() - A.Combustion:GetCooldown() <= VarHoldCombustionThreshold) then
            ...
            if A.RuneofPower:IsReady(unit) and (A.Firestarter:IsSpellLearned() and S.Firestarter:ActiveRemains() > A.RuneofPower:GetSpellChargesFullRechargeTime() or VarTimeToCombustion > VarCombustionRopCutoff and Unit("player"):HasBuffsDown(A.CombustionBuff.ID, true)) or Unit(unit):TimeToDie() < VarTimeToCombustion and Unit("player"):HasBuffsDown(A.CombustionBuff.ID, true)) or VarDisableCombustion) then
                return A.RuneofPower:Show(icon)
            end
            ...
            if A.BurstIsON(unit) and (not VarDisableCombustion and (A.RuneofPower:IsSpellLearned() and VarTimeToCombustion <= A.RuneofPower:GetSpellCastTime() or VarTimeToCombustion <= 0) and not S.Firestarter:ActiveStatus() or Unit("player"):HasBuffs(A.CombustionBuff.ID, true))) then
            ...
            if (Unit("player"):HasBuffs(A.RuneofPowerBuff.ID, true)) and Unit("player"):HasBuffsDown(A.CombustionBuff.ID, true))) then
            ...
            VarFireBlastPooling = num(A.RuneofPower:IsSpellLearned() and A.RuneofPower:GetCooldown() < A.FireBlast:FullRechargeTimeP() and (VarTimeToCombustion > VarCombustionRopCutoff or VarDisableCombustion or S.Firestarter:ActiveStatus()) and (A.RuneofPower:GetCooldown() < Unit(unit):TimeToDie() or A.RuneofPower:GetSpellCharges() > 0) or not VarDisableCombustion and VarTimeToCombustion < A.FireBlast:GetSpellChargesFullRechargeTime() + A.FireBlast:BaseDuration() * A.BlasterMaster:GetAzeriteRank() > 0 and not S.Firestarter:ActiveStatus() and VarTimeToCombustion < Unit(unit):TimeToDie() or A.Firestarter:IsSpellLearned() and S.Firestarter:ActiveStatus() and S.Firestarter:ActiveRemains() < A.FireBlast:FullRechargeTimeP() + A.FireBlast:BaseDuration() * A.BlasterMaster:GetAzeriteRank() > 0)
