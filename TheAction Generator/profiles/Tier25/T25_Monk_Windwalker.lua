...
local function EvaluateTargetIfBlackoutKick406(unit)
  return combo_strike and (A.RisingSunKick:GetCooldown() > 3 or Player:Chi() >= 3) and (A.FistsofFury:GetCooldown() > 4 or Player:Chi() >= 4 or (Player:Chi() == 2 and Unit("player"):GetSpellLastCast(A.TigerPalm)))
end
...
local function EvaluateTargetIfTigerPalm504(unit)
  return not combo_break and (Player:EnergyTimeToMaxPredicted() < 1 or (A.Serenity:IsSpellLearned() and A.Serenity:GetCooldown() < 2) or (Player:EnergyTimeToMaxPredicted() < 4 and A.FistsofFury:GetCooldown() < 1.5)) and Player:ChiMax() - Player:Chi() >= 2 and not Unit(unit):HasDeBuffs(A.TouchofDeathDebuff.ID, true)
end
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Cd(unit)
            ...
            if A.StormEarthandFire:IsReady(unit) and A.BurstIsON(unit) and (A.StormEarthandFire:GetSpellCharges() == 2 or (not Azerite:EssenceHasMajor(A.WorldveinResonance.ID) or (Unit("player"):HasBuffs(A.WorldveinResonanceBuff.ID, true)) or A.WorldveinResonance:GetCooldown() > A.StormEarthandFire:FullRechargeTimeP())) and (A.TouchofDeath:GetCooldown() > A.StormEarthandFire:FullRechargeTimeP() or A.TouchofDeath:GetCooldown() > Unit(unit):TimeToDie()) and A.FistsofFury:GetCooldown() <= 9 and Player:Chi() >= 3 and A.WhirlingDragonPunch:GetCooldown() <= 13 or Unit(unit):HasDeBuffs(A.TouchofDeathDebuff.ID, true) or Unit(unit):TimeToDie() < 20) then
                return A.StormEarthandFire:Show(icon)
            end
            
            -- blood_of_the_enemy,if=dot.touch_of_death.remains|target.time_to_die<12
            if A.BloodoftheEnemy:AutoHeartOfAzerothP(unit, true) and Action.GetToggle(1, "HeartOfAzeroth") and (Unit(unit):HasDeBuffs(A.TouchofDeathDebuff.ID, true) or Unit(unit):TimeToDie() < 12) then
            ...
            if A.AncestralCall:AutoRacial(unit) and Action.GetToggle(1, "Racial") and A.BurstIsON(unit) and (Unit(unit):HasDeBuffs(A.TouchofDeathDebuff.ID, true) or Unit(unit):TimeToDie() < 16) then
                return A.AncestralCall:Show(icon)
            end
            
            -- fireblood,if=dot.touch_of_death.remains|target.time_to_die<9
            if A.Fireblood:AutoRacial(unit) and Action.GetToggle(1, "Racial") and A.BurstIsON(unit) and (Unit(unit):HasDeBuffs(A.TouchofDeathDebuff.ID, true) or Unit(unit):TimeToDie() < 9) then
            ...
            if A.AshvanesRazorCoral:IsReady(unit) and (VarCoralDoubleTodOnUse and A.TouchofDeath:GetCooldown() >= 23 and (Unit(unit):HasDeBuffsDown(A.RazorCoralDebuff.ID, true)) or Unit("player"):HasBuffs(A.StormEarthandFireBuff.ID, true)) > 13 or Unit(unit):TimeToDie() - A.TouchofDeath:GetCooldown() < 40 and A.TouchofDeath:GetCooldown() < 23 or Unit(unit):TimeToDie() < 25)) then
            ...
            if A.AshvanesRazorCoral:IsReady(unit) and (not VarCoralDoubleTodOnUse and (Unit(unit):HasDeBuffsDown(A.RazorCoralDebuff.ID, true)) or (not A.DribblingInkpod:IsExists() or Unit(unit):TimeToDieX(30) < 8) and (Unit(unit):HasDeBuffs(A.TouchofDeathDebuff.ID, true) or A.TouchofDeath:GetCooldown() + 9 > Unit(unit):TimeToDie() and Unit("player"):HasBuffs(A.StormEarthandFireBuff.ID, true)) or Unit(unit):TimeToDie() < 25))) then
                return A.AshvanesRazorCoral:Show(icon)
            end
        ...
        local function Serenity(unit)
            ...
            if A.SpinningCraneKick:IsReady(unit) and (combo_strike and (MultiUnits:GetByRangeInCombat(8, 5, 10) >= 3 or (A.HitCombo:IsSpellLearned() and Unit("player"):GetSpellLastCast(A.BlackoutKick)) or (MultiUnits:GetByRangeInCombat(8, 5, 10) == 2 and Unit("player"):GetSpellLastCast(A.BlackoutKick)))) then
                return A.SpinningCraneKick:Show(icon)
            end
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.ProlongedPower:IsReady(unit) and Action.GetToggle(1, "Potion") and (Unit("player"):HasBuffs(A.SerenityBuff.ID, true)) or Unit(unit):HasDeBuffs(A.TouchofDeathDebuff.ID, true) or not A.Serenity:IsSpellLearned() and trinket.proc.agility.react or Unit("player"):HasHeroism() or Unit(unit):TimeToDie() <= 60) then
                return A.ProlongedPower:Show(icon)
            end
