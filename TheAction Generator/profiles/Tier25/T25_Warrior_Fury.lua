...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function SingleUnit(unit)(unit)
            ...
            if A.Bladestorm:IsReady(unit) and A.BurstIsON(unit) and (Player:PrevGCDP(1, A.Rampage)) then
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.AshvanesRazorCoral:IsReady(unit) and (Unit(unit):TimeToDie() < 20 or not Unit(unit):HasDeBuffs(A.RazorCoralDebuff.ID, true) or (Unit(unit):HealthPercent() < 30.1 and Unit(unit):HasDeBuffs(A.ConductiveInkDebuff.ID, true)) or (not Unit(unit):HasDeBuffs(A.ConductiveInkDebuff.ID, true) and Unit("player"):HasBuffs(A.MemoryofLucidDreamsBuff.ID, true) or Player:PrevGCDP(2, A.GuardianofAzeroth) or Player:PrevGCDP(2, A.Recklessness) and (not Azerite:EssenceHasMajor(A.MemoryofLucidDreams.ID) and not Azerite:EssenceHasMajor(A.CondensedLifeforce.ID)))) then
