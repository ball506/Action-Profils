...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Single(unit)
            ...
            if A.Flurry:IsReady(unit) and (A.Ebonbolt:IsSpellLearned() and Unit("player"):PrevGCDP(1, A.Ebonbolt) and Unit("player"):HasBuffsStacks(A.BrainFreezeBuff.ID, true)) then
                return A.Flurry:Show(icon)
            end
            
            -- flurry,if=prev_gcd.1.glacial_spike&buff.brain_freeze.react
            if A.Flurry:IsReady(unit) and (Unit("player"):PrevGCDP(1, A.GlacialSpike) and Unit("player"):HasBuffsStacks(A.BrainFreezeBuff.ID, true)) then
                return A.Flurry:Show(icon)
            end
