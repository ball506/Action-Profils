...
local function APL()
  ...
  S.MongooseBite = CurrentMongooseBite()
    ...
    if A.BattlePotionofAgility:IsReady(unit) and Action.GetToggle(1, "Potion") and (Unit("player"):HasBuffs(A.GuardianofAzerothBuff) and (Unit("player"):HasBuffs(A.BerserkingBuff) or Unit("player"):HasBuffs(A.BloodFuryBuff) or not Player:IsRace("Troll")) or (Unit(unit):HasBuffs(A.PotionofUnbridledFuryBuff) and Unit(unit):TimeToDie() < 61 or Unit(unit):TimeToDie() < 26) or not bool(essence.condensed_lifeforce.major) and Unit("player"):HasBuffs(A.CoordinatedAssaultBuff)) then
      A.BattlePotionofAgility:Show(icon)
    end
    ...
    if A.AshvanesRazorCoral:IsReady(unit) and (A.DribblingInkpod:IsEquipped and (bool(Target:HasDeBuffsDown(A.RazorCoralDeBuffDebuff)) or time_to_pct_30 < 1 or (health.pct < 30 and Unit("player"):HasBuffs(A.GuardianofAzerothBuff) or Unit("player"):HasBuffs(A.MemoryofLucidDreamsBuff))) or (not A.DribblingInkpod:IsEquipped and (Unit("player"):HasBuffs(A.MemoryofLucidDreamsBuff) or Unit("player"):HasBuffs(A.GuardianofAzerothBuff) and A.GuardianofAzeroth:GetCooldown() > 175) or bool(Target:HasDeBuffsDown(A.RazorCoralDeBuffDebuff))) or Unit(unit):TimeToDie() < 20) then
    ...
    if A.MemoryofLucidDreams:IsReady(unit) and (Player:Focus() < Player:FocusMax() - 30 and Unit("player"):HasBuffs(A.CoordinatedAssaultBuff)) then
    ...
    if A.KillCommand:IsReady(unit) and (Player:Focus() + Player:FocusCastRegen(A.KillCommand:GetSpellCastTime) < Player:FocusMax()) then
        return A.KillCommand:Show(icon)
    end
    -- steel_trap,if=focus+cast_regen<focus.max
    if A.SteelTrap:IsReady(unit) and (Player:Focus() + Player:FocusCastRegen(A.SteelTrap:GetSpellCastTime) < Player:FocusMax()) then
