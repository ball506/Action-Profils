...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Precombat(unit)
            ...
            if A.AspectoftheWild:IsReady(unit) and Unit("player"):HasBuffsDown(A.AspectoftheWildBuff) and (not A.PrimalInstincts:GetAzeriteRank() and not bool(essence.essence_of_the_focusing_iris.major) and (A.AzsharasFontofPower:IsExists or not A.CyclotronicBlast:IsExists)) then
            ...
            if A.BestialWrath:IsReady(unit) and Unit("player"):HasBuffsDown(A.BestialWrathBuff) and (A.PrimalInstincts:GetAzeriteRank() and not bool(essence.essence_of_the_focusing_iris.major) and (A.AzsharasFontofPower:IsExists or not A.CyclotronicBlast:IsExists)) then
        ...
        local function Cleave(unit)
            ...
            if A.CobraShot:IsReady(unit) and (A.KillCommand:GetCooldown() > Unit("player"):FocusTimeToMaxPredicted() and (MultiUnits:GetByRangeInCombat(40, 5, 10) < 3 or not A.RapidReload:GetAzeriteRank())) then
        ...
        local function St(unit)
            ...
            if A.CobraShot:IsReady(unit) and ((Unit("player"):Focus() - A.CobraShot:Cost() + Unit("player"):FocusRegen() * (A.KillCommand:GetCooldown() - 1) > A.KillCommand:Cost() or A.KillCommand:GetCooldown() > 1 + A.GetGCD() or Unit("player"):HasBuffs(A.MemoryofLucidDreamsBuff)) and A.KillCommand:GetCooldown() > 1) then
                return A.CobraShot:Show(icon)
            end
            -- spitting_cobra
            if A.SpittingCobra:IsReady(unit) then
        ...
        if inCombat and Unit(unit):IsExists() and not Unit(unit):IsTotem() then
            ...
            if A.AshvanesRazorCoral:IsReady(unit) and (Unit(unit):HasDeBuffs(A.RazorCoralDeBuffDebuff) and (Unit("player"):GetSpellLastCast(A.AspectoftheWild) or not A.CyclotronicBlast:IsExists and Unit("player"):HasBuffs(A.AspectoftheWildBuff)) and (Unit(unit):HealthPercent < 35 or not bool(essence.condensed_lifeforce.major)) or (bool(Unit(unit):HasDeBuffsDown(A.RazorCoralDeBuffDebuff)) or Unit(unit):TimeToDie() < 26) and Unit(unit):TimeToDie() > (24 * num((A.CyclotronicBlast:GetCooldown() + 4 < Unit(unit):TimeToDie())))) then
