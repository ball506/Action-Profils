...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Precombat(unit)
            ...
            if A.AdrenalineRush:IsReady(unit) and Unit("player"):HasBuffsDown(A.AdrenalineRushBuff) and ((not A.PocketsizedComputationDevice:IsExists or not bool(A.CyclotronicBlast:BaseDuration) or bool(raid_event.invulnerable.exists))) then
        ...
        local function Build(unit)
            ...
            if A.PistolShot:IsReady(unit) and (Unit("player"):HasBuffs(A.OpportunityBuff) and (Unit("player"):HasBuffsStacks(A.KeepYourWitsAboutYouBuff) < 14 or Unit("player"):HasBuffs(A.DeadshotBuff) or Unit("player"):EnergyPredicted() < 45)) then
        ...
        local function Cds(unit)
            ...
            if A.AdrenalineRush:IsReady(unit) and (not Unit("player"):HasBuffs(A.AdrenalineRushBuff) and Unit("player"):EnergyTimeToMaxPredicted() > 1 and (not A.AzsharasFontofPower:IsExists or A.LatentArcana:GetCooldown() > 20)) then
                return A.AdrenalineRush:Show(icon)
            end
            ...
            if A.:IsReady(unit) and (not Unit("player"):IsStealthedP(true, true) and bool(Unit("player"):HasBuffsDown(A.AdrenalineRushBuff)) and bool(Unit("player"):HasBuffsDown(A.MemoryofLucidDreamsBuff)) and Unit("player"):EnergyTimeToMaxPredicted() > 4 and RtB_Buffs < 5) then
                A.:Show(icon)
            end
        ...
        local function Essences(unit)
            ...
            if A.ConcentratedFlame:IsReady(unit) and (Unit("player"):EnergyTimeToMaxPredicted() > 1 and not Unit("player"):HasBuffs(A.BladeFlurryBuff) and (not Unit(unit):HasDeBuffs(A.ConcentratedFlameBurnDebuff) and not A.ConcentratedFlame:IsSpellInFlight() or A.ConcentratedFlame:FullRechargeTimeP() < A.GetGCD())) then
