...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Essences(unit)
            ...
            if A.BloodoftheEnemy:IsReady(unit) and (BurnPhase:On() and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff)) and bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and Unit("player"):ArcaneChargesP == Unit("player"):ArcaneChargesMax or Unit(unit):TimeToDie() < A.ArcanePower:GetCooldown()) then
            ...
            if A.ConcentratedFlame:IsReady(unit) and (bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff)) and (not BurnPhase:On() or Unit(unit):TimeToDie() < A.ArcanePower:GetCooldown()) and Unit("player"):ManaTimeToMaxPredicted() >= A.ConcentratedFlame:GetSpellCastTime()) then
            ...
            if A.MemoryofLucidDreams:IsReady(unit) and (not BurnPhase:On() and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff)) and bool(A.ArcanePower:GetCooldown()) and Unit("player"):ArcaneChargesP == Unit("player"):ArcaneChargesMax and (not A.RuneofPower:IsSpellLearned() or bool(A.RuneofPower:ChargesP())) or Unit(unit):TimeToDie() < A.ArcanePower:GetCooldown()) then
            ...
            if A.WorldveinResonance:IsReady(unit) and (BurnPhase:On() and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff)) and bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and Unit("player"):ArcaneChargesP == Unit("player"):ArcaneChargesMax or Unit(unit):TimeToDie() < A.ArcanePower:GetCooldown()) then
