...
A[3] = function(icon, isMulti)
   ...
   Everyone.AoEToggleEnemiesUpdate()
    ...
    if A.:IsReady(unit) and (bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and not bool(Unit("player"):HasBuffsStacks(A.ArcanePowerBuff)) and A.ArcanePower:GetCooldown() > 20) then
      A.:Show(icon)
    end
    ...
    if A.BloodoftheEnemy:IsReady(unit) and (BurnPhase:On() and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff)) and bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and Player:ArcaneChargesP == Player:ArcaneChargesMax or Unit(unit):TimeToDie() < A.ArcanePower:GetCooldown()) then
    ...
    if A.ConcentratedFlame:IsReady(unit) and (bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff)) and (not BurnPhase:On() or Unit(unit):TimeToDie() < A.ArcanePower:GetCooldown()) and Player:ManaTimeToMaxPredicted() >= A.ConcentratedFlame:GetSpellCastTime()) then
        return A.ConcentratedFlame:Show(icon)
    end
    -- focused_azerite_beam,if=buff.rune_of_power.down&buff.arcane_power.down
    if A.FocusedAzeriteBeam:IsReady(unit) and (bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff))) then
        return A.FocusedAzeriteBeam:Show(icon)
    end
    -- guardian_of_azeroth,if=buff.rune_of_power.down&buff.arcane_power.down
    if A.GuardianofAzeroth:IsReady(unit) and (bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff))) then
        return A.GuardianofAzeroth:Show(icon)
    end
    -- purifying_blast,if=buff.rune_of_power.down&buff.arcane_power.down
    if A.PurifyingBlast:IsReady(unit) and (bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff))) then
        return A.PurifyingBlast:Show(icon)
    end
    -- ripple_in_space,if=buff.rune_of_power.down&buff.arcane_power.down
    if A.RippleInSpace:IsReady(unit) and (bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff))) then
        return A.RippleInSpace:Show(icon)
    end
    -- the_unbound_force,if=buff.rune_of_power.down&buff.arcane_power.down
    if A.TheUnboundForce:IsReady(unit) and (bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff))) then
    ...
    if A.MemoryofLucidDreams:IsReady(unit) and (not BurnPhase:On() and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff)) and bool(A.ArcanePower:GetCooldown()) and Player:ArcaneChargesP == Player:ArcaneChargesMax and (not A.RuneofPower:IsSpellLearned() or bool(A.RuneofPower:ChargesP())) or Unit(unit):TimeToDie() < A.ArcanePower:GetCooldown()) then
    ...
    if A.WorldveinResonance:IsReady(unit) and (BurnPhase:On() and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff)) and bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and Player:ArcaneChargesP == Player:ArcaneChargesMax or Unit(unit):TimeToDie() < A.ArcanePower:GetCooldown()) then
  ...
  if not Player:AffectingCombat() and not Player:IsCasting() then
    ...
    if A.AzsharasFontofPower:IsReady(unit) and (bool(Unit("player"):HasBuffsDown(A.RuneofPowerBuff)) and bool(Unit("player"):HasBuffsDown(A.ArcanePowerBuff)) and (A.ArcanePower:GetCooldown() <= 4 + 10 * VarFontDoubleOnUse and A.Evocation:GetCooldown() <= VarAverageBurnLength + 4 + 10 * VarFontDoubleOnUse or Unit(unit):TimeToDie() < A.ArcanePower:GetCooldown())) then
      A.AzsharasFontofPower:Show(icon)
    end
