...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Cds(unit)
            ...
            if (not Unit("player"):IsStealthedP(true, true) and Unit(unit):HasDeBuffs(A.NightbladeDebuff)) then
            ...
            if A.ShurikenTornado:IsReady(unit) and (Unit("player"):EnergyPredicted() >= 60 and Unit(unit):HasDeBuffs(A.NightbladeDebuff) and A.SymbolsofDeath:HasCooldownUps and A.ShadowDance:ChargesP() >= 1) then
            ...
            if A.SymbolsofDeath:IsReady(unit) and (Unit(unit):HasDeBuffs(A.NightbladeDebuff) and not A.ShadowBlades:HasCooldownUps and (not A.ShurikenTornado:IsSpellLearned() or A.ShadowFocus:IsSpellLearned() or A.ShurikenTornado:GetCooldown() > 2) and (not bool(essence.blood_of_the_enemy.major) or A.BloodoftheEnemy:GetCooldown() > 2) and (A.NightsVengeance:GetAzeriteRank() < 2 or Unit("player"):HasBuffs(A.NightsVengeanceBuff))) then
                return A.SymbolsofDeath:Show(icon)
            end
            ...
            if A.ShadowBlades:IsReady(unit) and (not Unit("player"):IsStealthedP(true, true) and Unit(unit):HasDeBuffs(A.NightbladeDebuff) and Unit("player"):ComboPointsDeficit() >= 2) then
            ...
            if A.:IsReady(unit) and (not Unit("player"):IsStealthedP(true, true) and Unit(unit):HasDeBuffs(A.NightbladeDebuff) and not Unit("player"):HasBuffs(A.SymbolsofDeathBuff) and Unit("player"):EnergyDeficitPredicted() >= 30) then
                A.:Show(icon)
            end
        ...
        local function Essences(unit)
            ...
            if A.ConcentratedFlame:IsReady(unit) and (Unit("player"):EnergyTimeToMaxPredicted() > 1 and not Unit("player"):HasBuffs(A.SymbolsofDeathBuff) and (not Unit(unit):HasDeBuffs(A.ConcentratedFlameBurnDebuff) and not A.ConcentratedFlame:IsSpellInFlight() or A.ConcentratedFlame:FullRechargeTimeP() < A.GetGCD())) then
            ...
            if A.FocusedAzeriteBeam:IsReady(unit) and ((MultiUnits:GetByRangeInCombat(40, 5, 10) >= 2 or 10000000000 > 60) and not A.SymbolsofDeath:HasCooldownUps and not Unit("player"):HasBuffs(A.SymbolsofDeathBuff) and Unit("player"):EnergyDeficitPredicted() >= 30) then
            ...
            if A.MemoryofLucidDreams:IsReady(unit) and (Unit("player"):EnergyPredicted() < 40 and Unit("player"):HasBuffs(A.SymbolsofDeathBuff)) then
        ...
        local function StealthCds(unit)
            ...
            if A.Vanish:IsReady(unit) and (not bool(VarShdThreshold) and Unit("player"):ComboPointsDeficit() > 1 and Unit(unit):HasDeBuffs(A.FindWeaknessDebuff) < 1 and A.SymbolsofDeath:GetCooldown() >= 3) then
            ...
            if A.Shadowmeld:IsReady(unit) and A.BurstIsON(unit) and (Unit("player"):EnergyPredicted() >= 40 and Unit("player"):EnergyDeficitPredicted() >= 10 and not bool(VarShdThreshold) and Unit("player"):ComboPointsDeficit() > 1 and Unit(unit):HasDeBuffs(A.FindWeaknessDebuff) < 1) then
            ...
            if (bool(VarUsePriorityRotation) and (A.Nightstalker:IsSpellLearned() or A.DarkShadow:IsSpellLearned())) then
                VarShdComboPoints = num(Unit("player"):ComboPointsDeficit() <= 1 + 2 * num(A.TheFirstDance:GetAzeriteRank()))
            end
        ...
        local function Stealthed(unit)
            ...
            if (Unit("player"):HasBuffs(A.ShurikenTornadoBuff) and Unit("player"):ComboPointsDeficit() <= 2) then
            ...
            if (Unit("player"):ComboPointsDeficit() <= 1 - num((A.DeeperStratagem:IsSpellLearned() and (Unit("player"):HasBuffs(A.VanishBuff) or A.TheFirstDance:GetAzeriteRank() and not A.DarkShadow:IsSpellLearned() and not A.Subterfuge:IsSpellLearned() and MultiUnits:GetByRangeInCombat(40, 5, 10) < 3)))) then
                local ShouldReturn = Finish(unit); if ShouldReturn then return ShouldReturn; end
            end
        ...
        if inCombat and Unit(unit):IsExists() and not Unit(unit):IsTotem() then
            ...
            if A.Nightblade:IsReady(unit) and (A.NightsVengeance:GetAzeriteRank() and not Unit("player"):HasBuffs(A.NightsVengeanceBuff) and Unit("player"):ComboPointsDeficit() > 1 and (MultiUnits:GetByRangeInCombat(40, 5, 10) < 2 or bool(VarUsePriorityRotation)) and (A.SymbolsofDeath:GetCooldown() <= 3 or (A.NightsVengeance:GetAzeriteRank() >= 2 and Unit("player"):HasBuffs(A.SymbolsofDeathBuff) > 3 and not Unit("player"):IsStealthedP(true, true) and A.ShadowDance:ChargesFractionalP() >= 0.9))) then
                return A.Nightblade:Show(icon)
            end
