...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Cd(unit)
            ...
            if A.StormEarthandFire:IsReady(unit) and A.BurstIsON(unit) and (A.StormEarthandFire:GetSpellCharges() == 2 or (not bool(Azerite:EssenceHasMajor(A.WorldveinResonance.ID)) or (Unit("player"):HasBuffs(A.WorldveinResonanceBuff.ID, true) or A.WorldveinResonance:GetCooldown() > A.StormEarthandFire:FullRechargeTimeP())) and (A.TouchofDeath:GetCooldown() > A.StormEarthandFire:FullRechargeTimeP() or A.TouchofDeath:GetCooldown() > Unit(unit):TimeToDie()) and A.FistsofFury:GetCooldown() <= 9 and Player:Chi() >= 3 and A.WhirlingDragonPunch:GetCooldown() <= 13 or bool(Unit(unit):HasDeBuffs(A.TouchofDeathDebuff.ID, true)) or Unit(unit):TimeToDie() < 20) then
