...
local function APL()
  ...
  UpdateExecuteID()
        ...
        local function Execute(unit)
            ...
            if A.Skullsplitter:IsReady(unit) and (Unit("player"):Rage() < 60 and bool(Unit("player"):HasBuffsDown(A.DeadlyCalmBuff)) and bool(Unit("player"):HasBuffsDown(A.MemoryofLucidDreamsBuff))) then
            ...
            if A.Bladestorm:IsReady(unit) and A.BurstIsON(unit) and (not Unit("player"):HasBuffs(A.MemoryofLucidDreamsBuff) and Unit("player"):HasBuffs(A.TestofMightBuff) and Unit("player"):Rage() < 30 and not Unit("player"):HasBuffs(A.DeadlyCalmBuff)) then
        ...
        local function SingleUnit(unit)(unit)
            ...
            if A.Overpower:IsReady(unit) and (Unit("player"):Rage() < 30 and Unit("player"):HasBuffs(A.MemoryofLucidDreamsBuff) and Unit(unit):HasDeBuffs(A.ColossusSmashDebuff)) then
                return A.Overpower:Show(icon)
            end
            -- mortal_strike
  ...
  if Everyone.TargetIsValid() then
            ...
            if A.AshvanesRazorCoral:IsReady(unit) and (not Unit(unit):HasDeBuffs(A.RazorCoralDeBuffDebuff) or (Unit(unit):HealthPercent < 30.1 and Unit(unit):HasDeBuffs(A.ConductiveInkDeBuffDebuff)) or (not Unit(unit):HasDeBuffs(A.ConductiveInkDeBuffDebuff) and (Unit("player"):HasBuffs(A.MemoryofLucidDreamsBuff) or (Unit(unit):HasDeBuffs(A.ColossusSmashDebuff) and not bool(essence.memory_of_lucid_dreams.major))))) then
            ...
            if A.BloodoftheEnemy:IsReady(unit) and (Unit("player"):HasBuffs(A.TestofMightBuff) or (Unit(unit):HasDeBuffs(A.ColossusSmashDebuff) and not A.TestofMight:GetAzeriteRank())) then
                return A.BloodoftheEnemy:Show(icon)
            end
            -- purifying_blast,if=!debuff.colossus_smash.up&!buff.test_of_might.up
            if A.PurifyingBlast:IsReady(unit) and (not Unit(unit):HasDeBuffs(A.ColossusSmashDebuff) and not Unit("player"):HasBuffs(A.TestofMightBuff)) then
                return A.PurifyingBlast:Show(icon)
            end
            -- ripple_in_space,if=!debuff.colossus_smash.up&!buff.test_of_might.up
            if A.RippleInSpace:IsReady(unit) and (not Unit(unit):HasDeBuffs(A.ColossusSmashDebuff) and not Unit("player"):HasBuffs(A.TestofMightBuff)) then
                return A.RippleInSpace:Show(icon)
            end
            -- worldvein_resonance,if=!debuff.colossus_smash.up&!buff.test_of_might.up
            if A.WorldveinResonance:IsReady(unit) and (not Unit(unit):HasDeBuffs(A.ColossusSmashDebuff) and not Unit("player"):HasBuffs(A.TestofMightBuff)) then
                return A.WorldveinResonance:Show(icon)
            end
            -- focused_azerite_beam,if=!debuff.colossus_smash.up&!buff.test_of_might.up
            if A.FocusedAzeriteBeam:IsReady(unit) and (not Unit(unit):HasDeBuffs(A.ColossusSmashDebuff) and not Unit("player"):HasBuffs(A.TestofMightBuff)) then
                return A.FocusedAzeriteBeam:Show(icon)
            end
            -- concentrated_flame,if=!debuff.colossus_smash.up&!buff.test_of_might.up&dot.concentrated_flame_burn.remains=0
            if A.ConcentratedFlame:IsReady(unit) and (not Unit(unit):HasDeBuffs(A.ColossusSmashDebuff) and not Unit("player"):HasBuffs(A.TestofMightBuff) and Unit(unit):HasDeBuffs(A.ConcentratedFlameBurnDebuff) == 0) then
