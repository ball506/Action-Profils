...
A[3] = function(icon, isMulti)
   ...
   Everyone.AoEToggleEnemiesUpdate()
    ...
    if A.UnholyFrenzy:IsReady(unit) and (bool(essence.vision_of_perfection.enabled) or (bool(essence.condensed_lifeforce.enabled) and bool(pet.apoc_ghoul.active)) or Unit(unit):HasDeBuffsStacks(A.FesteringWoundDebuff) < 4 and not (A.RampingAmplitudeGigavoltEngine:IsEquipped or A.MagusoftheDead:GetAzeriteRank) or A.Apocalypse:GetCooldown() < 2 and (A.RampingAmplitudeGigavoltEngine:IsEquipped or A.MagusoftheDead:GetAzeriteRank)) then
    ...
    if A.MemoryofLucidDreams:IsReady(unit) and (Player:RuneTimeToX(1) > A.GetGCD() and Player:RunicPower() < 40) then
