...
A[3] = function(icon, isMulti)
   ...
   Everyone.AoEToggleEnemiesUpdate()
    ...
    if A.AshvanesRazorCoral:IsReady(unit) and (bool(Target:HasDeBuffsDown(A.RazorCoralDeBuffDebuff)) or (Unit("player"):HasBuffs(A.AvengingWrathBuff) >= 20 or Unit("player"):HasBuffsStacks(A.CrusadeBuff) == 10 and Unit("player"):HasBuffs(A.CrusadeBuff) > 15) and (A.GuardianofAzeroth:GetCooldown() > 90 or Unit(unit):TimeToDie() < 30 or not bool(essence.condensed_lifeforce.major))) then
    ...
    if A.WorldveinResonance:IsReady(unit) and (A.AvengingWrath:GetCooldown() < A.GetGCD() and Player:HolyPower() >= 3 or A.Crusade:GetCooldown() < A.GetGCD() and Player:HolyPower() >= 4 or A.AvengingWrath:GetCooldown() >= 45 or A.Crusade:GetCooldown() >= 45) then
    ...
    if A.MemoryofLucidDreams:IsReady(unit) and ((Unit("player"):HasBuffs(A.AvengingWrathBuff) or Unit("player"):HasBuffs(A.CrusadeBuff) and Unit("player"):HasBuffsStacks(A.CrusadeBuff) == 10) and Player:HolyPower() <= 3) then
    ...
    if A.AvengingWrath:IsReady(unit) and A.BurstIsON(unit) and ((not A.Inquisition:IsSpellLearned() or Unit("player"):HasBuffs(A.InquisitionBuff)) and Player:HolyPower() >= 3) then
    ...
    if (true) then
      VarWingsPool = num(not A.Item169314:IsEquipped and (not A.Crusade:IsSpellLearned() and A.AvengingWrath:GetCooldown() > A.GetGCD() * 3 or A.Crusade:GetCooldown() > A.GetGCD() * 3) or A.Item169314:IsEquipped and (not A.Crusade:IsSpellLearned() and A.AvengingWrath:GetCooldown() > A.GetGCD() * 6 or A.Crusade:GetCooldown() > A.GetGCD() * 6))
    end
    ...
    if (true) then
      VarDsCastable = num(MultiUnits:GetByRangeInCombat(40, 5, 10) >= 2 and not A.RighteousVerdict:IsSpellLearned() or MultiUnits:GetByRangeInCombat(40, 5, 10) >= 3 and A.RighteousVerdict:IsSpellLearned() or Unit("player"):HasBuffs(A.EmpyreanPowerBuff) and bool(Target:HasDeBuffsDown(A.JudgmentDebuff)) and bool(Unit("player"):HasBuffsDown(A.DivinePurposeBuff)) and bool(Unit("player"):HasBuffsDown(A.AvengingWrathAutocritBuff)))
    end
    ...
    if (Player:HolyPower() >= 5 or Unit("player"):HasBuffs(A.MemoryofLucidDreamsBuff) or Unit("player"):HasBuffs(A.SeethingRageBuff) or bool(Unit("player"):HasBuffsDown(A.InquisitionBuff)) and Player:HolyPower() >= 3) then
    ...
    if A.WakeofAshes:IsReady(unit) and ((not (MultiUnits:GetByRangeInCombat(40, 5, 10) > 1) or 10000000000 > 15 or MultiUnits:GetByRangeInCombat(40, 5, 10) >= 2) and (Player:HolyPower() <= 0 or Player:HolyPower() == 1 and A.BladeofJustice:GetCooldown() > A.GetGCD()) and (A.AvengingWrath:GetCooldown() > 10 or A.Crusade:IsSpellLearned() and A.Crusade:GetCooldown() > 10)) then
