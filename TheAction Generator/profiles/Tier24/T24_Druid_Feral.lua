...
A[3] = function(icon, isMulti)
   ...
   Everyone.AoEToggleEnemiesUpdate()
    ...
    if A.FocusedAzeriteBeam:IsReady(unit) and (MultiUnits:GetByRangeInCombat(40, 5, 10) > 1 or (10000000000 > 90 and Player:EnergyDeficitPredicted() >= 50)) then
    ...
    if A.AshvanesRazorCoral:IsReady(unit) and (bool(Target:HasDeBuffsDown(A.RazorCoralDeBuffDebuff)) or Target:HasDeBuffs(A.ConductiveInkDeBuffDebuff) and target.time_to_pct_30 < 1.5 or not Target:HasDeBuffs(A.ConductiveInkDeBuffDebuff) and (Target:HasDeBuffsStacks(A.RazorCoralDeBuffDebuff) >= 25 - 10 * num(Target:HasDeBuffs(A.BloodoftheEnemyDebuff)) or Unit(unit):TimeToDie() < 40) and Unit("player"):HasBuffs(A.TigersFuryBuff) > 10) then
    ...
    if A.:IsReady(unit) and ((Player:EnergyDeficitPredicted() >= Player:EnergyRegen() * 3) and bool(Unit("player"):HasBuffsDown(A.TigersFuryBuff)) and not A.JungleFury:GetAzeriteRank) then
      A.:Show(icon)
    end
    ...
    if A.:IsReady(unit) and (Player:EnergyDeficitPredicted() >= 50) then
      A.:Show(icon)
    end
  ...
  if not inCombat and Unit(unit):IsExists() and Action.GetToggle(1, "DBM") and unit ~= "mouseover" and not Unit(unit):IsTotem() then 
    ...
    if A.Regrowth:IsReady(unit) and (Player:ComboPoints() == 5 and Unit("player"):HasBuffs(A.PredatorySwiftnessBuff) and A.Bloodtalons:IsSpellLearned() and bool(Unit("player"):HasBuffsDown(A.BloodtalonsBuff))) then
        return A.Regrowth:Show(icon)
    end
