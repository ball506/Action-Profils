...
local function EvaluateCycleObliterate146(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and true and Unit("player"):RunicPowerDeficit() >= 25 and not A.Frostscythe:IsSpellLearned()
end

local function EvaluateCycleFrostStrike165(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and Unit("player"):RunicPowerDeficit() < 20 and not A.Frostscythe:IsSpellLearned() and A.PillarofFrost:GetCooldown() > 5
end
...
local function EvaluateCycleObliterate236(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and Unit("player"):RunicPower() <= 32 and not A.Frostscythe:IsSpellLearned()
end
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function ColdHeart(unit)
            ...
            if A.ChainsofIce:IsReady(unit) and ((Unit("player"):HasBuffs(A.PillarofFrostBuff) <= A.GetGCD() * (1 + num(A.FrostwyrmsFury:HasCooldownUps)) or Unit("player"):HasBuffs(A.PillarofFrostBuff) < Unit("player"):RuneTimeToX(3)) and Unit("player"):HasBuffs(A.PillarofFrostBuff) and (A.IcyCitadel:GetAzeriteRank() <= 1 or Unit("player"):HasBuffs(A.BreathofSindragosaBuff))) then
                return A.ChainsofIce:Show(icon)
            end
            ...
            if A.ChainsofIce:IsReady(unit) and ((Unit("player"):HasBuffs(A.IcyCitadelBuff) < 4 or Unit("player"):HasBuffs(A.IcyCitadelBuff) < Unit("player"):RuneTimeToX(3)) and Unit("player"):HasBuffs(A.IcyCitadelBuff) and A.IcyCitadel:GetAzeriteRank() >= 2 and not Unit("player"):HasBuffs(A.BreathofSindragosaBuff)) then
                return A.ChainsofIce:Show(icon)
            end
        ...
        local function Cooldowns(unit)
            ...
            if A.JesHowler:IsReady(unit) and ((A.LurkersInsidiousGift:IsExists and bool(Unit("player"):HasBuffs(A.PillarofFrostBuff))) or (not A.LurkersInsidiousGift:IsExists and Unit("player"):HasBuffs(A.PillarofFrostBuff) < 12 and Unit("player"):HasBuffs(A.PillarofFrostBuff))) then
            ...
            if A.GrongsPrimalRage:IsReady(unit) and (Unit("player"):Rune() <= 3 and not Unit("player"):HasBuffs(A.PillarofFrostBuff) and (not Unit("player"):HasBuffs(A.BreathofSindragosaBuff) or not A.BreathofSindragosa:IsSpellLearned())) then
            ...
            if A.EmpowerRuneWeapon:IsReady(unit) and (A.PillarofFrost:HasCooldownUps and not A.BreathofSindragosa:IsSpellLearned() and Unit("player"):RuneTimeToX(5) > A.GetGCD() and Unit("player"):RunicPowerDeficit() >= 10 or target.1.time_to_die < 20) then
            ...
            if A.EmpowerRuneWeapon:IsReady(unit) and ((A.PillarofFrost:HasCooldownUps or target.1.time_to_die < 20) and A.BreathofSindragosa:IsSpellLearned() and Unit("player"):RunicPower() > 60) then
                return A.EmpowerRuneWeapon:Show(icon)
            end
        ...
        local function Essences(unit)
            ...
            if A.MemoryofLucidDreams:IsReady(unit) and (Unit("player"):HasBuffs(A.EmpowerRuneWeaponBuff) < 5 and Unit("player"):HasBuffs(A.BreathofSindragosaBuff) or (Unit("player"):RuneTimeToX(2) > A.GetGCD() and Unit("player"):RunicPower() < 50)) then
