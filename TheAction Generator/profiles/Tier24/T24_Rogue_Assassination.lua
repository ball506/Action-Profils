...
A[3] = function(icon, isMulti)
   ...
   Everyone.AoEToggleEnemiesUpdate()
    ...
    if A.AzsharasFontofPower:IsReady(unit) and (not Player:IsStealthedP(true, true) and MasterAssassinRemains == 0 and num((A.Vendetta:GetCooldown() < ?cooldown.toxic_blade.remains)) < 10 + 10 * num(A.AshvanesRazorCoral:IsEquipped) and not Target:HasDeBuffs(A.VendettaDebuff) and not Target:HasDeBuffs(A.ToxicBladeDebuff)) then
      A.AzsharasFontofPower:Show(icon)
    end
    -- call_action_list,name=essences,if=!stealthed.all&dot.rupture.ticking&master_assassin_remains=0
    if (not Player:IsStealthedP(true, true) and Target:HasDeBuffs(A.RuptureDebuff) and MasterAssassinRemains == 0) then
    ...
    if (true) then
      VarVendettaFontCondition = num(not A.AzsharasFontofPower:IsEquipped or A.ShroudedSuffocation:GetAzeriteRank or bool(Unit(unit):HasDeBuffsDown(A.RazorCoralDeBuffDebuff)) or trinket.ashvanes_razor_coral.cooldown.remains < 10 and (A.ToxicBlade:GetCooldown() < 1 or Unit(unit):HasDeBuffs(A.ToxicBladeDebuff)))
    end
    ...
    if A.Vendetta:IsReady(unit) and (not Player:IsStealthedP(true, false) and Unit(unit):HasDeBuffs(A.RuptureDebuff) and not Unit(unit):HasDeBuffs(A.VendettaDebuff) and bool(VarVendettaSubterfugeCondition) and bool(VarVendettaNightstalkerCondition) and bool(VarVendettaFontCondition)) then
    ...
    if A.Vanish:IsReady(unit) and (A.Subterfuge:IsSpellLearned() and not Player:IsStealthedP(true, false) and A.Garrote:HasCooldownUps and (bool(VarSsVanishCondition) or not A.ShroudedSuffocation:GetAzeriteRank and Unit(unit):HasDeBuffsRefreshable(A.GarroteDebuff)) and Player:ComboPointsDeficit() >= num(((1 + 2 * num(A.ShroudedSuffocation:GetAzeriteRank)) * MultiUnits:GetByRangeInCombat(40, 5, 10)) > ?4) and 10000000000 > 12) then
    ...
    if A.ToxicBlade:IsReady(unit) and (Unit(unit):HasDeBuffs(A.RuptureDebuff) and (not A.AzsharasFontofPower:IsEquipped or A.Vendetta:GetCooldown() > 10)) then
    ...
    if A.AshvanesRazorCoral:IsReady(unit) and (bool(Unit(unit):HasDeBuffsDown(A.RazorCoralDeBuffDebuff)) or Unit(unit):HasDeBuffs(A.VendettaDebuff) > 10 - 4 * num(A.AzsharasFontofPower:IsEquipped) or Unit(unit):TimeToDie() < 20) then
    ...
    if A.:IsReady(unit) and (MasterAssassinRemains == 0 and not Unit(unit):HasDeBuffs(A.VendettaDebuff) and not Unit(unit):HasDeBuffs(A.ToxicBladeDebuff) and bool(Unit("player"):HasBuffsDown(A.MemoryofLucidDreamsBuff)) and Player:EnergyPredicted() < 80 and Unit(unit):HasDeBuffs(A.RuptureDebuff) > 4) then
      A.:Show(icon)
    end
    ...
    if A.FanofKnives:IsReady(unit) and (bool(VarUseFiller) and (Unit("player"):HasBuffsStacks(A.HiddenBladesBuff) >= 19 or (not bool(priority_rotation) and MultiUnits:GetByRangeInCombat(40, 5, 10) >= 4 + num((A.DoubleDose:GetAzeriteRank > 2)) + num(Player:IsStealthedP(true, false))))) then
        return A.FanofKnives:Show(icon)
    end
    ...
    if A.Rupture:IsReady(unit) and (not bool(VarSkipRupture) and Player:ComboPoints() >= 4 and Unit(unit):HasDeBuffsRefreshable(A.RuptureDebuff) and (Unit(unit):PMultiplier(A.Rupture) <= 1 or Unit(unit):HasDeBuffs(A.RuptureDebuff) <= A.RuptureDebuff:TickTime and MultiUnits:GetByRangeInCombat(40, 5, 10) >= 3 + num(A.ShroudedSuffocation:GetAzeriteRank)) and (not HL.Exsanguinated(Unit(unit), "Rupture") or Unit(unit):HasDeBuffs(A.RuptureDebuff) <= A.RuptureDebuff:TickTime * 2 and MultiUnits:GetByRangeInCombat(40, 5, 10) >= 3 + num(A.ShroudedSuffocation:GetAzeriteRank)) and Unit(unit):TimeToDie() - Unit(unit):HasDeBuffs(A.RuptureDebuff) > 4) then
        return A.Rupture:Show(icon)
    end
    ...
    if A.ConcentratedFlame:IsReady(unit) and (Player:EnergyTimeToMaxPredicted() > 1 and not Unit(unit):HasDeBuffs(A.VendettaDebuff) and (not Unit(unit):HasDeBuffs(A.ConcentratedFlameBurnDebuff) and not A.ConcentratedFlame:IsSpellInFlight() or A.ConcentratedFlame:FullRechargeTimeP() < A.GetGCD())) then
    ...
    if A.BloodoftheEnemy:IsReady(unit) and (Unit(unit):HasDeBuffs(A.VendettaDebuff) and (not A.ToxicBlade:IsSpellLearned() or Unit(unit):HasDeBuffs(A.ToxicBladeDebuff) and Player:ComboPointsDeficit() <= 1 or Unit(unit):HasDeBuffs(A.VendettaDebuff) <= 10) or Unit(unit):TimeToDie() <= 10) then
    ...
    if A.FocusedAzeriteBeam:IsReady(unit) and (MultiUnits:GetByRangeInCombat(40, 5, 10) >= 2 or 10000000000 > 60 and Player:EnergyPredicted() < 70) then
    ...
    if A.MemoryofLucidDreams:IsReady(unit) and (Player:EnergyPredicted() < 50 and not A.Vendetta:HasCooldownUps) then
    ...
    if A.Rupture:IsReady(unit) and (A.Subterfuge:IsSpellLearned() and A.ShroudedSuffocation:GetAzeriteRank and not Unit(unit):HasDeBuffs(A.RuptureDebuff) and bool(VarSingleTarget)) then
        return A.Rupture:Show(icon)
    end
    -- pool_resource,for_next=1
