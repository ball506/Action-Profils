...
A[3] = function(icon, isMulti)
   ...
   Everyone.AoEToggleEnemiesUpdate()
    ...
    if A.ArcaneBarrage:IsReady(unit) and (((Player:ArcaneChargesP == Player:ArcaneChargesMax) and ((Player:ManaPercentageP() <= VarConserveMana) or (A.ArcanePower:GetCooldown() > A.RuneofPower:FullRechargeTimeP() and Player:ManaPercentageP() <= VarConserveMana + 25)) or (A.ArcaneOrb:IsSpellLearned() and A.ArcaneOrb:GetCooldown() <= A.GetGCD() and A.ArcanePower:GetCooldown() > 10)) or Player:ManaPercentageP() <= (VarConserveMana - 10)) then
        return A.ArcaneBarrage:Show(icon)
    end
