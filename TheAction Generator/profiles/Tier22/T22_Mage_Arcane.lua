...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Burn(unit)
            ...
            if A.PresenceofMind:IsReady(unit) and A.BurstIsON(unit) and (Unit("player"):HasBuffs(A.RuneofPowerBuff) <= PresenceOfMindMax * A.ArcaneBlast:GetSpellCastTime() or Unit("player"):HasBuffs(A.ArcanePowerBuff) <= PresenceOfMindMax * A.ArcaneBlast:GetSpellCastTime()) then
        ...
        local function Conserve(unit)
            ...
            if A.ArcaneBarrage:IsReady(unit) and (((Unit("player"):ArcaneChargesP == Unit("player"):ArcaneChargesMax) and ((Unit("player"):ManaPercentageP() <= VarConserveMana) or (A.ArcanePower:GetCooldown() > A.RuneofPower:FullRechargeTimeP() and Unit("player"):ManaPercentageP() <= VarConserveMana + 25)) or (A.ArcaneOrb:IsSpellLearned() and A.ArcaneOrb:GetCooldown() <= A.GetGCD() and A.ArcanePower:GetCooldown() > 10)) or Unit("player"):ManaPercentageP() <= (VarConserveMana - 10)) then
                return A.ArcaneBarrage:Show(icon)
            end
