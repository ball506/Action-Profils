...
local function EvaluateCycleGarrote469(unit)
  return A.Subterfuge:IsSpellLearned() and A.ShroudedSuffocation:GetAzeriteRank() and Unit(unit):TimeToDie() > Unit(unit):HasDeBuffs(A.GarroteDebuff) and Unit("player"):ComboPointsDeficit() > 1
end
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Cds(unit)
            ...
            if A.Vanish:IsReady(unit) and (A.Subterfuge:IsSpellLearned() and (not A.Exsanguinate:IsSpellLearned() or not bool(VarSingleUnit(unit))) and not Unit("player"):IsStealthedP(true, false) and A.Garrote:HasCooldownUps and Unit(unit):HasDeBuffsRefreshable(A.GarroteDebuff) and (MultiUnits:GetByRangeInCombat(40, 5, 10) <= 3 and Unit("player"):ComboPointsDeficit() >= 1 + MultiUnits:GetByRangeInCombat(40, 5, 10) or MultiUnits:GetByRangeInCombat(40, 5, 10) >= 4 and Unit("player"):ComboPointsDeficit() >= 4)) then
                return A.Vanish:Show(icon)
            end
            -- vanish,if=talent.master_assassin.enabled&!stealthed.all&master_assassin_remains<=0&!dot.rupture.refreshable
            if A.Vanish:IsReady(unit) and (A.MasterAssassin:IsSpellLearned() and not Unit("player"):IsStealthedP(true, true) and MasterAssassinRemains <= 0 and not Unit(unit):HasDeBuffsRefreshable(A.RuptureDebuff)) then
                return A.Vanish:Show(icon)
            end
        ...
        local function Direct(unit)
            ...
            if A.FanofKnives:IsReady(unit) and (bool(VarUseFiller) and (Unit("player"):HasBuffsStacks(A.HiddenBladesBuff) >= 19 or MultiUnits:GetByRangeInCombat(40, 5, 10) >= 4 + num((A.DoubleDose:GetAzeriteRank() > 2)) + num(Unit("player"):IsStealthedP(true, false)))) then
                return A.FanofKnives:Show(icon)
            end
