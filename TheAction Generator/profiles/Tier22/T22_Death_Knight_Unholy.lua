...
local function EvaluateCycleOutbreak275(unit)
    return (dot.virulent_plague.tick_time_remains + tick_time <= Unit(unit):HasDeBuffs(A.VirulentPlagueDebuff)) and Unit(unit):HasDeBuffs(A.VirulentPlagueDebuff) <= A.GetGCD()
end
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Cooldowns(unit)
            ...
            if A.UnholyFrenzy:IsReady(unit) and (Unit(unit):HasDeBuffsStacks(A.FesteringWoundDebuff) < 4) then
        ...
        if inCombat and Unit(unit):IsExists() and not Unit(unit):IsTotem() then
            ...
            if A.ArcaneTorrent:IsReady(unit) and A.BurstIsON(unit) and (Unit("player"):RunicPowerDeficit() > 65 and (bool(A.SummonGargoyle:GetCooldown()) or not A.SummonGargoyle:IsSpellLearned()) and Unit("player"):RuneDeficit() >= 5) then
            ...
            if A.Berserking:IsReady(unit) and A.BurstIsON(unit) and (bool(pet.gargoyle.active) or not A.SummonGargoyle:IsSpellLearned()) then
            ...
            if A.JesHowler:IsReady(unit) and (bool(pet.gargoyle.active) or not A.SummonGargoyle:IsSpellLearned()) then
                A.JesHowler:Show(icon)
            end
            -- use_item,name=galecallers_beak,if=pet.gargoyle.active|!talent.summon_gargoyle.enabled
            if A.GalecallersBeak:IsReady(unit) and (bool(pet.gargoyle.active) or not A.SummonGargoyle:IsSpellLearned()) then
