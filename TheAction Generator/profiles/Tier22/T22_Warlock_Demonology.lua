...
if not inCombat and Unit(unit):IsExists() and Action.GetToggle(1, "DBM") and unit ~= "mouseover" and not Unit(unit):IsTotem() then
  local ShouldReturn = Precombat(unit); if ShouldReturn then return ShouldReturn; end
        ...
        if inCombat and Unit(unit):IsExists() and not Unit(unit):IsTotem() then
            ...
            if A.CallDreadstalkers:IsReady(unit) and (A.Item132369:IsExists or (A.SummonDemonicTyrant:GetCooldown() < 9 and bool(Unit("player"):HasBuffs(A.DemonicCallingBuff))) or (A.SummonDemonicTyrant:GetCooldown() < 11 and not bool(Unit("player"):HasBuffs(A.DemonicCallingBuff))) or A.SummonDemonicTyrant:GetCooldown() > 14) then
                return A.CallDreadstalkers:Show(icon)
            end
            ...
            if A.HandofGuldan:IsReady(unit) and (Unit("player"):SoulShardsP >= 5 or (Unit("player"):SoulShardsP >= 3 and A.CallDreadstalkers:GetCooldown() > 4 and (not A.SummonVilefiend:IsSpellLearned() or A.SummonVilefiend:GetCooldown() > 3))) then
                return A.HandofGuldan:Show(icon)
            end
            -- soul_strike,if=soul_shard<5&buff.demonic_core.stack<=2
            if A.SoulStrike:IsReady(unit) and (Unit("player"):SoulShardsP < 5 and Unit("player"):HasBuffsStacks(A.DemonicCoreBuff) <= 2) then
