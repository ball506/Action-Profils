...
local function EvaluateTargetIfFilterNemesis33(unit)
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Cooldown(unit)
            ...
            if A.Metamorphosis:IsReady(unit) and (A.Demonic:IsSpellLearned() and (not bool(A.ChaoticTransformation:GetAzeriteRank()) or (A.EyeBeam:GetCooldown() > 20 and A.BladeDance:GetCooldown() > A.GetGCD()))) then
                return A.Metamorphosis:Show(icon)
            end
            -- nemesis,target_if=min:target.time_to_die,if=raid_event.adds.exists&debuff.nemesis.down&(active_enemies>desired_targets|raid_event.adds.in>60)
            if A.Nemesis:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Nemesis, 40, "min", EvaluateTargetIfFilterNemesis33, EvaluateTargetIfNemesis48) then 
