...
local function EvaluateCycleFrostStrike57(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and Unit("player"):RunicPowerDeficit() < (15 + num(A.RunicAttenuation:IsSpellLearned()) * 3) and not A.Frostscythe:IsSpellLearned()
end

local function EvaluateCycleObliterate80(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and Unit("player"):RunicPowerDeficit() > (25 + num(A.RunicAttenuation:IsSpellLearned()) * 3) and not A.Frostscythe:IsSpellLearned()
end
...
local function EvaluateCycleObliterate124(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and Unit("player"):RuneTimeToX(4) < A.GetGCD() and Unit("player"):RunicPowerDeficit() >= 25 and not A.Frostscythe:IsSpellLearned()
end

local function EvaluateCycleFrostStrike143(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and Unit("player"):RunicPowerDeficit() < 20 and A.PillarofFrost:GetCooldown() > Unit("player"):RuneTimeToX(4) and not A.Frostscythe:IsSpellLearned()
end

local function EvaluateCycleObliterate172(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and Unit("player"):RunicPowerDeficit() >= (35 + num(A.RunicAttenuation:IsSpellLearned()) * 3) and not A.Frostscythe:IsSpellLearned()
end

local function EvaluateCycleFrostStrike195(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and A.PillarofFrost:GetCooldown() > Unit("player"):RuneTimeToX(4) and Unit("player"):RunicPowerDeficit() < 40 and not A.Frostscythe:IsSpellLearned()
end

local function EvaluateCycleObliterate214(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and Unit("player"):RunicPower() <= 30 and not A.Frostscythe:IsSpellLearned()
end

local function EvaluateCycleObliterate237(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and Unit("player"):RuneTimeToX(5) < A.GetGCD() or Unit("player"):RunicPower() <= 45 and not A.Frostscythe:IsSpellLearned()
end

local function EvaluateCycleObliterate262(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and Unit("player"):RunicPowerDeficit() > 25 or Unit("player"):Rune() > 3 and not A.Frostscythe:IsSpellLearned()
end
...
local function EvaluateCycleFrostStrike500(unit)
    return (Unit(unit):HasDeBuffsStacks(A.RazoriceDebuff) < 5 or Unit(unit):HasDeBuffs(A.RazoriceDebuff) < 10) and not Unit("player"):HasBuffs(A.RimeBuff) or Unit("player"):RunicPowerDeficit() < 10 or Unit("player"):RuneTimeToX(2) > A.GetGCD() and not A.Frostscythe:IsSpellLearned()
end
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function BosPooling(unit)
            ...
            if A.Obliterate:IsReady(unit) and (Unit("player"):RuneTimeToX(4) < A.GetGCD() and Unit("player"):RunicPowerDeficit() >= 25) then
                return A.Obliterate:Show(icon)
            end
            ...
            if A.GlacialAdvance:IsReady(unit) and (Unit("player"):RunicPowerDeficit() < 20 and A.PillarofFrost:GetCooldown() > Unit("player"):RuneTimeToX(4) and MultiUnits:GetByRangeInCombat(40, 5, 10) >= 2) then
                return A.GlacialAdvance:Show(icon)
            end
            ...
            if A.FrostStrike:IsReady(unit) and (Unit("player"):RunicPowerDeficit() < 20 and A.PillarofFrost:GetCooldown() > Unit("player"):RuneTimeToX(4)) then
                return A.FrostStrike:Show(icon)
            end
            ...
            if A.Frostscythe:IsReady(unit) and (Unit("player"):HasBuffs(A.KillingMachineBuff) and Unit("player"):RunicPowerDeficit() > (15 + num(A.RunicAttenuation:IsSpellLearned()) * 3) and MultiUnits:GetByRangeInCombat(40, 5, 10) >= 2) then
                return A.Frostscythe:Show(icon)
            end
        ...
        local function BosTicking(unit)
            ...
            if A.Obliterate:IsReady(unit) and (Unit("player"):RunicPower() <= 30) then
                return A.Obliterate:Show(icon)
            end
            ...
            if A.HornofWinter:IsReady(unit) and (Unit("player"):RunicPowerDeficit() >= 30 and Unit("player"):RuneTimeToX(3) > A.GetGCD()) then
                return A.HornofWinter:Show(icon)
            end
            -- remorseless_winter
            if A.RemorselessWinter:IsReady(unit) then
            ...
            if A.ArcaneTorrent:IsReady(unit) and A.BurstIsON(unit) and (Unit("player"):RunicPowerDeficit() > 20) then
                return A.ArcaneTorrent:Show(icon)
            end
        end
        local function ColdHeart(unit)
            ...
            if A.ChainsofIce:IsReady(unit) and ((Unit("player"):HasBuffs(A.PillarofFrostBuff) <= A.GetGCD() * (1 + num(A.FrostwyrmsFury:HasCooldownUps)) or Unit("player"):HasBuffs(A.PillarofFrostBuff) < Unit("player"):RuneTimeToX(3)) and Unit("player"):HasBuffs(A.PillarofFrostBuff) and A.IcyCitadel:GetAzeriteRank() <= 2) then
                return A.ChainsofIce:Show(icon)
            end
            ...
            if A.ChainsofIce:IsReady(unit) and ((Unit("player"):HasBuffs(A.IcyCitadelBuff) <= A.GetGCD() * (1 + num(A.FrostwyrmsFury:HasCooldownUps)) or Unit("player"):HasBuffs(A.IcyCitadelBuff) < Unit("player"):RuneTimeToX(3)) and Unit("player"):HasBuffs(A.IcyCitadelBuff) and A.IcyCitadel:GetAzeriteRank() and A.IcyCitadel:GetAzeriteRank() > 2) then
                return A.ChainsofIce:Show(icon)
            end
        ...
        local function Cooldowns(unit)
            ...
            if A.GrongsPrimalRage:IsReady(unit) and (Unit("player"):Rune() <= 3 and not Unit("player"):HasBuffs(A.PillarofFrostBuff) and (not Unit(unit):HasDeBuffs(A.BreathofSindragosaDebuff) or not A.BreathofSindragosa:IsSpellLearned())) then
            ...
            if A.EmpowerRuneWeapon:IsReady(unit) and (A.PillarofFrost:HasCooldownUps and not A.BreathofSindragosa:IsSpellLearned() and Unit("player"):RuneTimeToX(5) > A.GetGCD() and Unit("player"):RunicPowerDeficit() >= 10 or Unit(unit):TimeToDie() < 20) then
            ...
            if A.EmpowerRuneWeapon:IsReady(unit) and ((A.PillarofFrost:HasCooldownUps or Unit(unit):TimeToDie() < 20) and A.BreathofSindragosa:IsSpellLearned() and Unit("player"):Rune() >= 3 and Unit("player"):RunicPower() > 60) then
                return A.EmpowerRuneWeapon:Show(icon)
            end
        ...
        local function Obliteration(unit)
            ...
            if A.GlacialAdvance:IsReady(unit) and ((not Unit("player"):HasBuffs(A.RimeBuff) or Unit("player"):RunicPowerDeficit() < 10 or Unit("player"):RuneTimeToX(2) > A.GetGCD()) and MultiUnits:GetByRangeInCombat(40, 5, 10) >= 2) then
                return A.GlacialAdvance:Show(icon)
            end
            ...
            if A.FrostStrike:IsReady(unit) and (not Unit("player"):HasBuffs(A.RimeBuff) or Unit("player"):RunicPowerDeficit() < 10 or Unit("player"):RuneTimeToX(2) > A.GetGCD()) then
                return A.FrostStrike:Show(icon)
            end
            -- howling_blast,if=buff.rime.up
            if A.HowlingBlast:IsReady(unit) and (Unit("player"):HasBuffs(A.RimeBuff)) then
        ...
        local function Standard(unit)
            ...
            if A.Frostscythe:IsReady(unit) and (Unit("player"):HasBuffs(A.KillingMachineBuff) and Unit("player"):RuneTimeToX(4) >= A.GetGCD()) then
                return A.Frostscythe:Show(icon)
            end
            -- obliterate,if=runic_power.deficit>(25+talent.runic_attenuation.enabled*3)
            if A.Obliterate:IsReady(unit) and (Unit("player"):RunicPowerDeficit() > (25 + num(A.RunicAttenuation:IsSpellLearned()) * 3)) then
