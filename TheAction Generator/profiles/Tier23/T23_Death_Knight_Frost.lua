...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Obliteration(unit)
            ...
            if A.Obliterate:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Obliterate, 10, "min", EvaluateCycleObliterate426) then
                    return A.Obliterate:Show(icon) 
                end
            end
            ...
            if A.Obliterate:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Obliterate, 10, "min", EvaluateCycleObliterate459) then
                    return A.Obliterate:Show(icon) 
                end
            end
            ...
            if A.FrostStrike:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FrostStrike, 10, "min", EvaluateCycleFrostStrike500) then
                    return A.FrostStrike:Show(icon) 
                end
            end
            ...
            if A.Obliterate:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Obliterate, 10, "min", EvaluateCycleObliterate523) then
                    return A.Obliterate:Show(icon) 
                end
            end
            -- obliterate
