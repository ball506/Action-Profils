...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function DconOpener(unit)
        ...
        local function Implosion(unit)
            ...
            if A.HandofGuldan:IsReady(unit) and (Unit("player"):SoulShardsP >= 3 and (((Unit("player"):GetSpellLastCast(A.HandofGuldan) or Unit("player"):HasBuffsStacks(A.WildImpsBuff) >= 3) and Unit("player"):HasBuffsStacks(A.WildImpsBuff) < 9) or A.SummonDemonicTyrant:GetCooldown() <= A.GetGCD() * 2 or Unit("player"):HasBuffs(A.DemonicPowerBuff) > A.GetGCD() * 2)) then
                return A.HandofGuldan:Show(icon)
            end
            ...
            if A.Demonbolt:IsReady(unit) and (Unit("player"):GetSpellLastCast(A.HandofGuldan) and Unit("player"):SoulShardsP >= 1 and (Unit("player"):HasBuffsStacks(A.WildImpsBuff) <= 3 or Unit("player"):GetSpellLastCast(A.HandofGuldan)) and Unit("player"):SoulShardsP < 4 and Unit("player"):HasBuffs(A.DemonicCoreBuff)) then
                return A.Demonbolt:Show(icon)
            end
            ...
            if A.SoulStrike:IsReady(unit) and (Unit("player"):SoulShardsP < 5 and Unit("player"):HasBuffsStacks(A.DemonicCoreBuff) <= 2) then
                return A.SoulStrike:Show(icon)
            end
            -- demonbolt,if=soul_shard<=3&buff.demonic_core.up&(buff.demonic_core.stack>=3|buff.demonic_core.remains<=gcd*5.7)
            if A.Demonbolt:IsReady(unit) and (Unit("player"):SoulShardsP <= 3 and Unit("player"):HasBuffs(A.DemonicCoreBuff) and (Unit("player"):HasBuffsStacks(A.DemonicCoreBuff) >= 3 or Unit("player"):HasBuffs(A.DemonicCoreBuff) <= A.GetGCD() * 5.7)) then
                return A.Demonbolt:Show(icon)
            end
        ...
        local function NetherPortalActive(unit)
            ...
            if A.SummonDemonicTyrant:IsReady(unit) and (Unit("player"):HasBuffs(A.NetherPortalBuff) < 5 and Unit("player"):SoulShardsP == 0) then
                return A.SummonDemonicTyrant:Show(icon)
            end
            ...
            if A.Demonbolt:IsReady(unit) and (Unit("player"):HasBuffs(A.DemonicCoreBuff) and Unit("player"):SoulShardsP <= 3) then
                return A.Demonbolt:Show(icon)
            end
            -- call_action_list,name=build_a_shard
            if (true) then
        ...
        local function NetherPortalBuilding(unit)
            ...
            if A.NetherPortal:IsReady(unit) and (Unit("player"):SoulShardsP >= 5 and (not A.PowerSiphon:IsSpellLearned() or Unit("player"):HasBuffs(A.DemonicCoreBuff))) then
            ...
            if A.HandofGuldan:IsReady(unit) and (A.CallDreadstalkers:GetCooldown() > 18 and Unit("player"):SoulShardsP >= 3) then
                return A.HandofGuldan:Show(icon)
            end
            -- power_siphon,if=buff.wild_imps.stack>=2&buff.demonic_core.stack<=2&buff.demonic_power.down&soul_shard>=3
            if A.PowerSiphon:IsReady(unit) and (Unit("player"):HasBuffsStacks(A.WildImpsBuff) >= 2 and Unit("player"):HasBuffsStacks(A.DemonicCoreBuff) <= 2 and bool(Unit("player"):HasBuffsDown(A.DemonicPowerBuff)) and Unit("player"):SoulShardsP >= 3) then
...
if not inCombat and Unit(unit):IsExists() and Action.GetToggle(1, "DBM") and unit ~= "mouseover" and not Unit(unit):IsTotem() then
  local ShouldReturn = Precombat(unit); if ShouldReturn then return ShouldReturn; end
        ...
        if inCombat and Unit(unit):IsExists() and not Unit(unit):IsTotem() then
            ...
            if (A.DemonicConsumption:IsSpellLearned() and Unit("player"):CombatTime < 30 and not bool(A.SummonDemonicTyrant:GetCooldown())) then
                local ShouldReturn = DconOpener(unit); if ShouldReturn then return ShouldReturn; end
            end
            ...
            if A.Demonbolt:IsReady(unit) and (Unit("player"):SoulShardsP <= 3 and Unit("player"):HasBuffs(A.DemonicCoreBuff) and Unit("player"):HasBuffsStacks(A.DemonicCoreBuff) == 4) then
                return A.Demonbolt:Show(icon)
            end
            ...
            if A.SummonDemonicTyrant:IsReady(unit) and (Unit("player"):SoulShardsP < 3 and (not A.DemonicConsumption:IsSpellLearned() or Unit("player"):HasBuffsStacks(A.WildImpsBuff) + imps_spawned_during.2000 / Unit("player"):SpellHaste >= 6 and time_to_imps.all.remains < A.SummonDemonicTyrant:GetSpellCastTime()) or Unit(unit):TimeToDie() < 20) then
                return A.SummonDemonicTyrant:Show(icon)
            end
