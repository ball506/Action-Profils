    ...
    local function EnemyRotation(unit)
        ...
        local function Direct(unit)
            ...
            if A.FanofKnives:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FanofKnives, 10, "min", EvaluateCycleFanofKnives175) then
                    return A.FanofKnives:Show(icon) 
                end
            end
            ...
            if A.Mutilate:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Mutilate, 40, "min", EvaluateCycleMutilate196) then
        ...
        local function Dot(unit)
            ...
            if A.Garrote:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Garrote, 40, "min", EvaluateCycleGarrote225) then
            ...
            if A.Rupture:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Rupture, 5, "min", EvaluateCycleRupture314) then
                    return A.Rupture:Show(icon) 
                end
            end
        end
        ...
        local function Stealthed(unit)
            ...
            if A.Garrote:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Garrote, 40, "min", EvaluateCycleGarrote391) then
                    return A.Garrote:Show(icon) 
                end
            end
            -- garrote,cycle_targets=1,if=talent.subterfuge.enabled&remains<=10&pmultiplier<=1&target.time_to_die-remains>2
            if A.Garrote:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Garrote, 40, "min", EvaluateCycleGarrote412) then
                    return A.Garrote:Show(icon) 
                end
            end
            ...
            if A.Garrote:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Garrote, 40, "min", EvaluateCycleGarrote447) then
                    return A.Garrote:Show(icon) 
                end
            end
            -- pool_resource,for_next=1
