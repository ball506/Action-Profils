...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Standard(unit)
            ...
            if A.DeathStrike:IsReady(unit) and (Unit("player"):RunicPowerDeficit() <= (15 + num(Unit("player"):HasBuffs(A.DancingRuneWeaponBuff)) * 5 + MultiUnits:GetByRangeInCombat(40, 5, 10) * num(A.Heartbreaker:IsSpellLearned()) * 2) or Unit(unit):TimeToDie() < 10) then
                return A.DeathStrike:Show(icon)
            end
