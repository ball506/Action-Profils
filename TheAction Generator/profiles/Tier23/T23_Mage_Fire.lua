...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function CombustionPhase(unit)
            ...
            if (bool(A.BlasterMaster:GetAzeriteRank()) and A.FlameOn:IsSpellLearned() and not bool(essence.memory_of_lucid_dreams.enabled)) then
            ...
            if ((bool(A.BlasterMaster:GetAzeriteRank()) and Unit("player"):HasBuffsStacks(A.BlasterMasterBuff.ID, true) >= 3) or not bool(A.BlasterMaster:GetAzeriteRank())) then
                local ShouldReturn = ActiveTalents(unit); if ShouldReturn then return ShouldReturn; end
            end
            ...
            if A.Combustion:IsReady(unit) and A.BurstIsON(unit) and (bool(essence.memory_of_lucid_dreams.enabled) and (Unit("player"):HasBuffs(A.RuneofPowerBuff.ID, true) or not A.RuneofPower:IsSpellLearned())) then
                return A.Combustion:Show(icon)
            end
            -- potion
            if A.BattlePotionofIntellect:IsReady(unit) and Action.GetToggle(1, "Potion") then
            ...
            if A.FireBlast:IsReady(unit) and (not bool(essence.memory_of_lucid_dreams.enabled) and (not bool(A.BlasterMaster:GetAzeriteRank()) or not A.FlameOn:IsSpellLearned()) and ((Unit("player"):HasBuffs(A.CombustionBuff.ID, true) and (bool(Unit("player"):HasBuffsStacks(A.HeatingUpBuff.ID, true)) and not A.Pyroblast:IsSpellInFlight() and not bool(action.scorch.executing)) or (bool(action.scorch.execute_remains) and bool(Unit("player"):HasBuffsDown(A.HeatingUpBuff.ID, true)) and bool(Unit("player"):HasBuffsDown(A.HotStreakBuff.ID, true)) and not A.Pyroblast:IsSpellInFlight())))) then
                return A.FireBlast:Show(icon)
            end
            -- pyroblast,if=prev_gcd.1.scorch&buff.heating_up.up
            if A.Pyroblast:IsReady(unit) and (Unit("player"):GetSpellLastCast(A.Scorch) and Unit("player"):HasBuffs(A.HeatingUpBuff.ID, true)) then
        ...
        local function StandardRotation(unit)
            ...
            if A.TidestormCodex:IsReady(unit) and (A.Combustion:GetCooldown() > 20 or A.Firestarter:IsSpellLearned() and S.Firestarter:ActiveRemains() > 20) then
...
if not Player:AffectingCombat() and not Player:IsCasting() then
  local ShouldReturn = Precombat(unit); if ShouldReturn then return ShouldReturn; end
        ...
        if inCombat and Unit(unit):IsExists() and not Unit(unit):IsTotem() then
            ...
            if A.MalformedHeraldsLegwraps:IsReady(unit) and (A.Combustion:GetCooldown() > 55) then
