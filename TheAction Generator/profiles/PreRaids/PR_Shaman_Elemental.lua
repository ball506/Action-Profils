...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function SingleUnit(unit)(unit)
            ...
            if A.Earthquake:IsReady(unit) and (MultiUnits:GetByRangeInCombat(40, 5, 10) > 1 and MultiUnits:GetByRangeInCombat(40, 5, 10) > 1 and (not A.SurgeofPower:IsSpellLearned() or not Unit(unit):HasDeBuffsRefreshable(A.FlameShockDebuff) or A.StormElemental:GetCooldown() > 120) and (not A.MasteroftheElements:IsSpellLearned() or Unit("player"):HasBuffs(A.MasteroftheElementsBuff) or Unit("player"):Maelstrom() >= 92)) then
                return A.Earthquake:Show(icon)
            end
            ...
            if A.EarthShock:IsReady(unit) and (not Unit("player"):HasBuffs(A.SurgeofPowerBuff) and A.MasteroftheElements:IsSpellLearned() and (Unit("player"):HasBuffs(A.MasteroftheElementsBuff) or Unit("player"):Maelstrom() >= 92 + 30 * num(A.CalltheThunder:IsSpellLearned()) or Unit("player"):HasBuffs(A.StormkeeperBuff) and MultiUnits:GetByRangeInCombat(40, 5, 10) < 2) or not A.MasteroftheElements:IsSpellLearned() and (Unit("player"):HasBuffs(A.StormkeeperBuff) or Unit("player"):Maelstrom() >= 90 + 30 * num(A.CalltheThunder:IsSpellLearned()) or not (A.StormElemental:GetCooldown() > 120 and A.StormElemental:IsSpellLearned()) and expected_combat_length - Unit("player"):CombatTime - A.StormElemental:GetCooldown() - 150 * math.floor ((expected_combat_length - Unit("player"):CombatTime - A.StormElemental:GetCooldown()) / 150) >= 30 * (1 + num((A.EchooftheElementals:GetAzeriteRank() >= 2))))) then
