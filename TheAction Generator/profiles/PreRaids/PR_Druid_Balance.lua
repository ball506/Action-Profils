...
local function EvaluateCycleSunfire208(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.SunfireDebuff)) and (bool(ap_check) and math.floor (Unit(unit):TimeToDie() / (2 * Unit("player"):SpellHaste)) * MultiUnits:GetByRangeInCombat(40, 5, 10) >= math.ceil (math.floor (2 / MultiUnits:GetByRangeInCombat(40, 5, 10)) * 1.5) + 2 * MultiUnits:GetByRangeInCombat(40, 5, 10) and (MultiUnits:GetByRangeInCombat(40, 5, 10) > 1 + num(A.TwinMoons:IsSpellLearned()) or Unit(unit):HasDeBuffs(A.MoonfireDebuff)) and (not bool(VarAzSs) or not Unit("player"):HasBuffs(A.CaIncBuff) or not bool(prev.sunfire)))
end

local function EvaluateCycleMoonfire261(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.MoonfireDebuff)) and (bool(ap_check) and math.floor (Unit(unit):TimeToDie() / (2 * Unit("player"):SpellHaste)) * MultiUnits:GetByRangeInCombat(40, 5, 10) >= 6 and (not bool(VarAzSs) or not Unit("player"):HasBuffs(A.CaIncBuff) or not bool(prev.moonfire)))
end

local function EvaluateCycleStellarFlare286(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.StellarFlareDebuff)) and (bool(ap_check) and math.floor (Unit(unit):TimeToDie() / (2 * Unit("player"):SpellHaste)) >= 5 and (not bool(VarAzSs) or not Unit("player"):HasBuffs(A.CaIncBuff) or not bool(prev.stellar_flare)))
end
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Precombat(unit)
            ...
            if (A.TwinMoons:IsSpellLearned() and (A.ArcanicPulsar:GetAzeriteRank() or A.Starlord:IsSpellLearned())) then
            ...
            if (not A.ArcanicPulsar:GetAzeriteRank() and not A.Starlord:IsSpellLearned() and A.StellarDrift:IsSpellLearned()) then
                VarSfTargets = VarSfTargets - 1
            end
            -- moonkin_form
        ...
        if inCombat and Unit(unit):IsExists() and not Unit(unit):IsTotem() then
            ...
            if A.CelestialAlignment:IsReady(unit) and (FutureAstralPower >= 40 and (not A.LivelySpirit:GetAzeriteRank() or Unit("player"):HasBuffs(A.LivelySpiritBuff)) and (Unit("player"):HasBuffsStacks(A.StarlordBuff) >= 2 or not A.Starlord:IsSpellLearned() or not bool(VarAzSs))) then
            ...
            if (Unit("player"):HasBuffs(A.StarlordBuff) < 8 and not bool(solar_wrath.ap_check)) then
            ...
            if A.LunarStrike:IsReady(unit) and (Unit("player"):HasBuffsStacks(A.SolarEmpowermentBuff) < 3 and (bool(ap_check) or Unit("player"):HasBuffsStacks(A.LunarEmpowermentBuff) == 3) and ((Unit("player"):HasBuffs(A.WarriorofEluneBuff) or Unit("player"):HasBuffs(A.LunarEmpowermentBuff) or MultiUnits:GetByRangeInCombat(40, 5, 10) >= 2 and not Unit("player"):HasBuffs(A.SolarEmpowermentBuff)) and (not bool(VarAzSs) or not Unit("player"):HasBuffs(A.CaIncBuff) or (not bool(prev.lunar_strike) and not A.Incarnation:IsSpellLearned() or bool(prev.solar_wrath))) or bool(VarAzSs) and Unit("player"):HasBuffs(A.CaIncBuff) and bool(prev.solar_wrath))) then
