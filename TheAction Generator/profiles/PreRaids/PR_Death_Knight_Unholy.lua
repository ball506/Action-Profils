...
A[3] = function(icon, isMulti)
  ...
  if not inCombat and Unit(unit):IsExists() and Action.GetToggle(1, "DBM") and unit ~= "mouseover" and not Unit(unit):IsTotem() then 
    ...
    if A.ArcaneTorrent:IsReady(unit) and A.BurstIsON(unit) and (Player:RunicPowerDeficit() > 65 and (bool(A.SummonGargoyle:GetCooldown()) or not A.SummonGargoyle:IsSpellLearned()) and Player:RuneDeficit() >= 5) then
