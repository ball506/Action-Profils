...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        if inCombat and Unit(unit):IsExists() and not Unit(unit):IsTotem() then
            ...
            if A.ArcaneTorrent:AutoRacial(unit) and A.BurstIsON(unit) and (Unit("player"):RunicPowerDeficit() > 65 and (bool(A.SummonGargoyle:GetCooldown()) or not A.SummonGargoyle:IsSpellLearned()) and Unit("player"):RuneDeficit() >= 5) then
            ...
            if A.Berserking:AutoRacial(unit) and A.BurstIsON(unit) and (bool(pet.gargoyle.active) or not A.SummonGargoyle:IsSpellLearned()) then
