...
svframe:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" and arg1 == "TheAction" then
...
function RenameHidePvPRatings()
    ...
    if not isOffseason then    --  Check if PvP season is active. 0 = active, 1 = inactive. 
        PVPQueueFrame:SetScript("OnUpdate", function(self, arg1)
            if isEnabled == "true" then
                if isPvPMasked == "true" then
                    for i = 1, 4 do 
...
anonymizerframe = StdUi:Window(UIParent, 400, 400, 'Title');
anonymizerframe:SetPoint('CENTER');
anonymizerframe:SetShown(false)

local anonymizerframetitle = StdUi:FontString(anonymizerframe, 'Anonymizer Test');
StdUi:GlueTop(anonymizerframetitle, anonymizerframe, 0, -40);
anonymizerframe:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" and arg1 == "TheAction" then
...
local l = StdUi:Label(anonymizerframe, 'Anonymize your self !', 20, nil, 160);
l:SetJustifyH('MIDDLE');
StdUi:GlueBelow(l, anonymizerframetitle, 0, -10);

-- Edit Box Guild Name
inputGuildName = StdUi:SimpleEditBox(anonymizerframe, 100, 24);
inputGuildName:SetPoint('CENTER');
StdUi:AddLabel(anonymizerframe, inputGuildName, 'Guild Name', 'TOP');

-- Edit Box Character Name
inputCharacterName = StdUi:SimpleEditBox(anonymizerframe, 100, 24);
StdUi:GlueBelow(inputCharacterName, inputGuildName, 0, -30);
StdUi:AddLabel(anonymizerframe, inputCharacterName, 'Character Name', 'TOP');

local button = StdUi:Button(anonymizerframe, 200, 20, 'Hide my ass');
StdUi:GlueBelow(button, inputCharacterName, 0, -50);
button:SetScript('OnClick', function()
    --RegisterNewGuildName(inputGuildName:GetText())
    --RegisterNewCharacterName(inputCharacterName:GetText())
	--PlayerFrame.name:SetText(inputCharacterName:GetText())	
	RenameDoHide()
end);
